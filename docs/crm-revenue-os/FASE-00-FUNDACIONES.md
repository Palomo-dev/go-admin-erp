...
| GET | `/api/crm/jobs` (real r2) | sesión (`getServerOrgContext(request)`: header `X-Organization-Id`/cookie) + rol **super admin, `STAGE_MANAGER_ROLE_IDS` (1/2/5) o cargo/rol con `admin.full_access` (`hasOrgAdminOrPermission` → `check_user_permission`, fail-closed; r4)** | `?status=&kind=&page=&pageSize=` (≤200) | `{success, items: JobListItem[], total, page, pageSize, stats:{byStatus,byKind,queuedOverdue}, recentFailed[≤50], canRetry}`; `payload_preview` **redactado** (solo `*_id`, `kind`, `campaign_id`, `event_type`, `entity_type`, `reason`, `resync`, `scheduled_for`, `retried_from`; ≤500 chars) | 401/400 org/403 rol | n/a |
| POST | `/api/crm/jobs/[id]/retry` (real r2) | sesión + `canRetryJobs` (r4: super admin, rol 1/2 o cargo/rol con `admin.full_access` por `check_user_permission`; Manager NO) | – | `{success, jobId, status:'queued'}` | 400 id; 403; 404 si no es de la org; 409 si no está en `dead|failed` | Sí: reutiliza el `dedupe_key` original (o `job:{id}:retry:{attempts}`) |
...
| `twilio/verify/{send,check}` | sesión + rate limit **5 / 10 min** por IP, user y `to` (cifra única con §7.1 y el código; F0-SEC r2-D: `Map` por instancia + store persistente y atómico `fn_rate_limit_hit`/`rate_limit_buckets` con `RATE_LIMIT_STORE=db`, fail-closed si el backend falla; migración pendiente de aplicar) | C1 msg |
...
| Rate-limit de `POST /api/crm/config/providers/test` por instancia (QA F0-REG r1, medio 14) | Usa `checkRateLimit` de `src/lib/security/rateLimit.ts` (5/min/org) sin `persistentCount`: en Vercel cada lambda cuenta aparte, así que el techo real es 5 × instancias activas. Riesgo aceptado: la prueba ejecuta la llamada más barata de cada SDK (gratuita) y solo la puede lanzar un admin autenticado; cuando exista un almacén compartido (tabla `rate_limits` o Upstash) se inyecta `persistentCount` sin tocar la ruta. Con `source: 'env'` la respuesta ya no revela datos de la cuenta de la plataforma |

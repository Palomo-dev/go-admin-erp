...
| `src/lib/utils/orgContext.ts:128` `getServerOrgContext(req?)` | ✅ r1 SEC | Header `X-Organization-Id` → cookie `goadmin_org_id` → `profiles.last_org_id` (consulta directa por `organization_id`, sin `limit(2)`, tester #7) → única membresía → 400 `ORG_AMBIGUOUS` (:178). `requireOrgAdmin` :201 (solo `is_super_admin` o `role_id ∈ {1, 2}` vía `orgAdmin.ts`; los nombres de rol no participan — F0-SEC C+D r3), `withOrg` :286, `withCron` :305 (C21 msg cerrado) |
...
| `vercel.json` crons + `src/app/api/crm/jobs/run/route.ts` | ✅ r3 JOBS | 3 crons a `/api/crm/jobs/run` (`*/2 * * * *` desde `9c0288a7`, `*/5`, `30 8`); contrato único en `src/lib/jobs/schedule.ts` (`DRAIN_INTERVAL_MIN`, `VERCEL_SCHEDULE_KINDS`) vigilado por el guardarraíl 18; ruta fail-closed, kinds inválidos ⇒ 400, productor programado con presupuesto antes de drenar (`budget_exhausted` si lo agota) |
...
Scheduler PRIMARIO (r3): Vercel Cron. pg_cron = RESPALDO manual (17/18/19 active=false; ver crm_v4_f00_41, pendiente).
pg_cron 'crm-jobs-every-minute' (* * * * *)  [job 17, hoy active=false; crm_v4_f00_41 lo pasa a */2 para igualar a Vercel]
  └─ fn_crm_cron_post('/api/crm/jobs/run') → net.http_post {vault:crm_app_url}/api/crm/jobs/run  body '{}'
       ...
       └─ runJobs(): deadline = 50 s menos lo que gaste el productor programado; si quedan < 2 s NO se drena
                      ({claimed:0, reason:'budget_exhausted'}; el drenaje */2 recoge lo encolado) — r3 N-1
...
pg_cron 'crm-daily-maintenance' (30 8 * * *)  [job 19, inactivo] → fn_crm_cron_post('/api/crm/jobs/run', '{"kinds":["recording_cleanup","maintenance"]}')
                                 (08:30 UTC = 03:30 Bogotá; pg_cron usa GMT. DB-r2 `crm_v4_f00_15`. HOY NO es espejo de vercel.json:
                                  le faltan health_recalculate y renewals_sync; `crm_v4_f00_41` (pendiente) los añade)
Vercel cron (PRIMARIO, vercel.json, Bearer automático): '*/2 * * * *' (todos; `DRAIN_SCHEDULE`) · '*/5 * * * *' (campaign_batch) ·
                                 '30 8 * * *' (recording_cleanup,maintenance,health_recalculate,renewals_sync) — se distinguen por el
                                 header x-vercel-cron-schedule; tabla única en src/lib/jobs/schedule.ts (guardarraíl 18 ↔ vercel.json)
Productor de recording_cleanup (r3, N-1/N-6/N-7): solo orgs activas CON grabaciones `ready` vencidas (call_recordings.retention_until,
  tipo date, < día UTC del productor ⊇ día de cualquier org); dedupe y `scheduled_for` con el DÍA DE LA ORGANIZACIÓN
  (organizations.timezone); bucle con presupuesto/signal ⇒ {enqueued, orgs, truncated?, pending_org_ids?}.
...
Todas las migraciones siguen `docs/POLITICA-MIGRACIONES.md` (vigente desde 2026-09-09): se aplican **solo** con `apply_migration` del MCP y, en el **mismo commit**, dejan su `.sql` en `supabase/migrations/<version>_<nombre>.sql` (exactamente como se aplicó) y su reversión en `supabase/rollbacks/<version>_<nombre>_rollback.sql`. La regla anterior de esta sección («prohibido crear `.sql` en el repo») quedó derogada; las 55 migraciones `crm_v4_*` aplicadas antes de versionarse se reconstruyeron desde `supabase_migrations.schema_migrations` en la ronda 3 de QA de F0-DB (2026-09-15, ver §13.1). Idempotentes. Patrón RLS copiado de `calls` (verificado en `pg_policies`): rol `authenticated`, `organization_id IN (SELECT om.organization_id FROM organization_members om WHERE om.user_id = (select auth.uid()) AND om.is_active = true)`. Desde la ronda 2 (`crm_v4_f00_13`) las políticas de F0 usan `(select auth.uid())` (initplan, advisor `auth_rls_initplan`), no el literal `auth.uid()` de las políticas antiguas.
...
> | F0-SEC A+B (qa r3, migración B) | `crm_v4_f00_40_cerrar_rpc_crm_anon` (`20260915223000`) | **APLICADA** (verificado por el QA r3 el 2026-09-16 con un `SELECT` por el MCP): las `fn_reporte_crm_*` dejan de ser ejecutables por `anon` y llevan guarda de pertenencia (`ORG_FORBIDDEN`); el asistente de reportes ya las llama con el cliente de sesión. Funciones `SECURITY DEFINER` ejecutables por `anon`: **216 → 184**. Quedan 184 (ver `docs/hallazgos/F-11.md`). En la misma verificación: `fn_rate_limit_hit` y `rate_limit_buckets` **existen** (`20260916000000_crm_v4_f0sec_rate_limit_buckets`, F0-SEC C+D); la nota 7c del tester r3 («no existe») quedó obsoleta. Pendiente fuera del código: `RATE_LIMIT_STORE=db` en Vercel, sin el cual el rate limit sigue en memoria por instancia. |
>
...
| GET | `/api/crm/jobs` (real r2) | sesión (`getServerOrgContext(request)`: header `X-Organization-Id`/cookie) + permiso **`crm.jobs.view`** (r5: `resolveJobsPermissions` → `hasOrgAdminOrPermission` → `check_user_permission`, fail-closed; super admin y roles 1/2 sin RPC; por defecto lo tienen los roles 1, 2 y 5 según `f00_45`, y un cargo lo concede o niega con precedencia). Cierre r5 (QA r5 N-1, opción b): reintentar exige ambos códigos (`canRetry = view ∧ retry`); `resolveJobsPermissions` consulta `crm.jobs.view` primero y `view=false` cierra la cola con 1 RPC (no se pregunta `retry`); `view=true` consulta después `crm.jobs.retry` (2 RPC, orden `[view, retry]`) y `canRetry` de la respuesta es esa conjunción | `?status=&kind=&page=&pageSize=` (≤200) | `{success, items: JobListItem[], total, page, pageSize, stats:{byStatus,byKind,queuedOverdue}, recentFailed[≤50], canRetry}`; `payload_preview` **redactado** (solo `*_id`, `kind`, `campaign_id`, `event_type`, `entity_type`, `reason`, `resync`, `scheduled_for`, `retried_from`; ≤500 chars) | 401/400 org/403 rol | n/a |
| POST | `/api/crm/jobs/[id]/retry` (real r2) | sesión + `readOrgBody` (regla 5) + permisos **`crm.jobs.view` Y `crm.jobs.retry`** (cierre r5: `resolveJobsPermissions(ctx).canRetry` → `hasOrgAdminOrPermission` → `check_user_permission`, fail-closed, mismo punto único que la GET; super admin y roles 1/2 sin RPC; por defecto los tienen los roles 1, 2 y 5 según `f00_45`: el Manager reintenta si además puede ver —decisión del dueño—, y un cargo que le niegue cualquiera de los dos códigos se lo impide; 403 «Requiere permisos crm.jobs.view y crm.jobs.retry») | – | `{success, jobId, status:'queued'}` | 400 id; 403; 404 si no es de la org; 409 si no está en `dead|failed` | Sí: reutiliza el `dedupe_key` original (o `job:{id}:retry:{attempts}`) |
...
| POST | `/api/integrations/twilio/verify/{send,check}` (modificar) | sesión + rate limit 5/10 min por IP, user y `to` (cifra unificada en F0-SEC r2) | igual | igual | 401, 429 | – |
...
| `src/lib/security/rateLimit.ts` (no estaba en §4.2) | `checkRateLimit` (ventana fija en memoria por instancia), `checkRateLimits`, `getClientIp`; nivel 2 persistente y atómico por `opts.store` (`rateLimitStore.ts`, RPC `fn_rate_limit_hit` sobre `rate_limit_buckets`) | El `persistentCount` inyectable (legado) se RETIRÓ en F0-SEC C+D r3: sin consumidor y con carrera; el único mecanismo persistente es el store atómico |
...
| `whatsapp` / `sms` | F16 / F8 | `{message_request, customer_id}` (real, F16) / `{comm_usage_log_id}` | 3 (real) / 5; error Meta 131049 → `JobRetryableError(retryAfterSeconds: 86400)`. **Idempotencia r3 (N-4):** `messages` no tiene `status` ni `provider_message_id` (MCP 2026-09-15) y el envío lo dispara el INSERT, así que el `{message_id}`+`pending` del plan no es implementable; el handler fija `clientRequestId = job:{id}` (hereda `retried_from`) y salta con `{skipped:true, reason:'already_sent'}` si `findByClientRequestId` ya encuentra el mensaje |
...
Reglas del runner (reales, r3): `deadlineMs = 50000` menos lo que consuma el productor programado (tope `PRODUCER_BUDGET_MS = 48 s`; cada paso recibe como máximo lo que quede; si quedan < 2 s no se drena: `reason:'budget_exhausted'`); lotes de 25 (≤200); timeout por job `min(30 s, deadline restante)` con `AbortController` — **advisory**: el handler sigue vivo hasta mirar `signal`. **r4 (T-1, contrato honesto):** `whatsapp`/`email`/`transcribe` comprueban `signal.aborted` al entrar y antes de cada efecto (`whatsapp`: tras `findByClientRequestId`; `email`: tras comprobar que `email_message_id` pertenece a la org; `transcribe`: solo al entrar, porque la pertenencia, la transcripción existente y el cobro los hace `transcribeCall` en una sola secuencia). Un abort **durante** el efecto no lo cancela (`sendWhatsApp`, `dispatchScheduledEmail` y `runTranscribePipeline` no admiten `signal`): lo cubre la idempotencia (`client_request_id` / `status_sent`+`provider_message_id` / guarda `processing` de `call_transcripts`). **r4 (T-2):** `findByClientRequestId` es fail-closed (propaga `error`; el handler lo convierte en `JobRetryableError('idempotency_check_failed')` sin enviar) y `retryJob` hereda `retried_from` de la raíz en cualquier generación; índice parcial `crm_v4_f00_42` (pendiente). **r4 (T-6):** entre los dos intentos de `fn_complete_job` se esperan `COMPLETE_RETRY_DELAY_MS = 300 ms` si queda presupuesto; resultado por job → `fn_complete_job`/`fn_fail_job` (un array no es registro plano ⇒ `p_result null`, N-9); **`fn_complete_job` fallido tras handler exitoso (N-4)**: reintento único de la RPC y, si persiste, `fn_fail_job(-1)` ⇒ `failed` terminal con `last_error='complete_failed: …'` y `summary.completeFailed` — nunca `queued` (el reclaim de 10 min re-ejecutaría el efecto); si también falla `fn_fail_job`, queda `running` para el reclaim; **job repetido en la misma invocación (N-8)**: se libera con `fn_release_job` y el bucle termina; lo no ejecutado se libera con `fn_release_job` (o fallback `attempts-1`); `worker` del body solo `/^[A-Za-z0-9._:-]{1,100}$/` (N-10); log JSON con `worker, org_id, job_id, kind, attempt, outcome, ms, error`. `max_attempts` reales: `crm_event` encolado por `fn_emit_crm_event` = 5 (default de la RPC), por resync/retry = 3; `crm_events.attempts` (DB-r2) limita el resync a 3 intentos de procesamiento por evento.
...
- `recording_cleanup` (r2): productor diario por org (`recording_cleanup:{yyyy-mm-dd}`, orgs con `comm_settings.is_active`) que solo encola cuando F3 registre el handler real (`hasRealJobHandler`); con el placeholder de F0 responde `reason:'handler_not_registered'` sin ensuciar la cola. **Ya activo**: con el handler de F3 registrado, una llamada a `?kind=recording_cleanup` encoló y completó un job por org activa (verificado 2026-09-09 en `outbound_jobs`, `dedupe_key='recording_cleanup:2026-09-09'`, todos `done`). **r3 (N-1/N-6/N-7):** se encola solo para las orgs activas que además tienen `call_recordings` `ready` con `retention_until` vencido (83 de 84 jobs/día eran `deleted:0`); la clave diaria y `payload.scheduled_for` usan el día calendario de la organización (`organizations.timezone`, `src/lib/jobs/orgTimezone.ts`), igual que el `cutoff` del handler (`retention_until` es `date`); el bucle tiene presupuesto y `signal` (`truncated`, `pending_org_ids`). **r4 (T-3/T-4/T-5):** con presupuesto 0 o señal abortada el productor no hace ninguna consulta (`reason:'budget_exhausted'`); las tareas F11 ya no reciben un mínimo de 1 s (con el total agotado no arrancan: `{ok:false, reason:'budget_exhausted'}`); `pending_org_ids` **no se persiste**: solo va al log/respuesta, y el día siguiente se reselecciona por grabaciones vencidas en el mismo orden (id asc) — la promesa «van primero al día siguiente» se retiró; rotación/persistencia para r5 si el truncado llegara a repetirse (hoy 1 org con grabaciones; ~600 caben en 48 s). **Supuestos (T-7/T-8):** la selección «por día UTC es superconjunto» vale a la hora real del cron (08:30 UTC; una org UTC+9 a las 23:00 UTC ya estaría en el día siguiente), y si `loadOrgTimezones` falla todas las orgs se encolan con `DEFAULT_TIMEZONE` (un `warn`); sin impacto con 84/84 orgs en `America/Bogota`.
- Liberación por deadline (tester F-4): `fn_release_job(p_job_id, p_worker)` (DB-r2) resta el intento del claim sin tocar `last_error`; desde `crm_v4_f00_37` (F0-DB r3) además cuenta `outbound_jobs.releases`, reprograma con backoff `30 s·2^(releases-1)` (tope 15 min, nunca `run_at = now()`) y mata el job (`dead`, `last_error = released_limit: …`) al llegar a `max_attempts*2`; el fallback UPDATE del runner replica la regla (`releaseBackoffSeconds`/`releaseCap` en `runner.ts`, test «tope de liberaciones» en `runner.test.ts`); hasta que exista, fallback `UPDATE … SET status='queued', attempts=attempts_del_claim-1 WHERE status='running' AND locked_by=worker` (`releasedWithoutRpc` en el resumen).
...
| `twilio/verify/{send,check}` | sesión + rate limit **5 / 10 min** por IP, user y `to` (cifra única con §7.1 y el código; F0-SEC r2-D: `Map` por instancia + store persistente y atómico `fn_rate_limit_hit`/`rate_limit_buckets` con `RATE_LIMIT_STORE=db`, fail-closed si el backend falla; migración `20260916000000_crm_v4_f0sec_rate_limit_buckets.sql` APLICADA el 2026-09-16 por el MCP; pendiente `RATE_LIMIT_STORE=db` en Vercel para salir del modo memoria) | C1 msg |
...
- [x] ~~Cero `.sql` nuevos en el repo.~~ Sustituido por `docs/POLITICA-MIGRACIONES.md`: cada migración aplicada por MCP lleva su `.sql` + rollback en el mismo commit (F0-DB r3, 2026-09-15).
...
| `channel_credentials` (tokens de WhatsApp/Facebook/Instagram del módulo chat) llega al navegador — riesgo ACEPTADO temporalmente, fuera de REG (QA F0-REG r1, alto 7) | `facebookChannelService.ts:159`, `instagramChannelService.ts:160`, `whatsappChannelService.ts:190` e `integrationsService.ts:801` hacen `.select('credentials')` con el cliente de navegador y las tarjetas `*CredentialsCard` (`'use client'`) los muestran. RLS lo limita a `organization_members.is_super_admin = true`, así que solo el administrador que los pegó vuelve a verlos, pero contradice §2.4 («el navegador nunca lee la columna `credentials`»). Decisión 2026-09-15: NO lo cierra F0-REG (es código del módulo chat, 2026-01/08, y el registry de REG no lo usa: `provider_configs` es la única tabla de credenciales del CRM y ya está cerrada por privilegios de columna). Pasa a SEC como tarea propia: mover la lectura a un endpoint server-only que devuelva las claves enmascaradas (`••••` + últimos 4) y la escritura a un `PUT` con service role; mientras tanto, riesgo aceptado con la mitigación de RLS por `is_super_admin`. Se documenta aquí para que el siguiente QA no lo cuente como olvido |
| Rate-limit de `POST /api/crm/config/providers/test` por instancia (QA F0-REG r1, medio 14) | Usa `checkRateLimit` de `src/lib/security/rateLimit.ts` (5/min/org) sin `store`: en Vercel cada lambda cuenta aparte, así que el techo real es 5 × instancias activas. Riesgo aceptado: la prueba ejecuta la llamada más barata de cada SDK (gratuita) y solo la puede lanzar un admin autenticado; el almacén compartido ya existe (`rate_limit_buckets` + `fn_rate_limit_hit`, F0-SEC C+D): basta pasar `{ store: getRateLimitStore() }` en la ruta cuando se active `RATE_LIMIT_STORE=db`. Con `source: 'env'` la respuesta ya no revela datos de la cuenta de la plataforma |
...
**Ronda 3 (JOBS, real).** Creados: `src/lib/jobs/schedule.ts` · `src/lib/jobs/orgTimezone.ts` · `src/lib/jobs/__tests__/builderR3.test.ts` · `supabase/migrations/20260915233000_crm_v4_f00_41_pg_cron_alineado_con_vercel.sql` (+ rollback; **pendiente de aplicar**). Modificados: `src/lib/jobs/{scheduler,runner,types}.ts` · `src/lib/jobs/handlers/{whatsapp,email,transcribe,recordingCleanup}.ts` · `src/app/api/crm/jobs/run/route.ts` · `src/lib/services/crm/jobsService.ts` · `src/components/crm/config/{JobsMonitor,JobsTable}.tsx` · `src/__tests__/guardrails.test.ts` (caso 18) · `src/lib/jobs/__tests__/{scheduler,runRoute,jobsService,testerR2Runner,testerR2Route}.test.ts`.

**Ronda 4 (JOBS, real).** Creados: `src/lib/jobs/__tests__/builderR4.test.ts` · `supabase/migrations/20260915234000_crm_v4_f00_42_idx_messages_client_request_id.sql` (+ rollback; **pendiente de aplicar**). Modificados: `src/lib/jobs/{scheduler,runner}.ts` · `src/lib/jobs/handlers/{whatsapp,email,transcribe}.ts` · `src/app/api/crm/jobs/{route,[id]/retry/route,run/route}.ts` · `src/lib/services/crm/jobsService.ts` · `src/lib/services/crm/whatsapp/outboundService.ts` (`findByClientRequestId` fail-closed, F16) · `src/lib/jobs/__tests__/{testerR3,testerR3Runner,builderR3,scheduler,jobsService}.test.ts`.
...
fn_release_job(p_job_id uuid, p_worker text) RETURNS boolean   -- ronda 2; tope y backoff en crm_v4_f00_37 (F0-DB QA r3)
  -- true solo si status='running' AND locked_by=p_worker: attempts=GREATEST(attempts-1,0), lock NULL, releases=releases+1,
  -- run_at = now() + least(30 s · 2^(releases-1), 900 s) (nunca now()); si releases >= max_attempts*2 → status='dead' y
  -- last_error='released_limit: N liberaciones por deadline (worker …)'. Columna nueva outbound_jobs.releases int NOT NULL DEFAULT 0.
  -- (para jobs reclamados que el runner no llega a procesar por deadline; no consume intento, pero no gira indefinidamente)
fn_can_contact(p_org integer, p_customer uuid, p_channel text, p_purpose text DEFAULT 'utility') RETURNS boolean   -- authenticated + service_role
  -- p_channel se normaliza con lower(trim(...)) (crm_v4_f00_37: 'EMAIL' == 'email'); con auth.role()='authenticated' exige pertenencia
  -- activa a p_org (ya no es oráculo entre organizaciones; service_role pasa igual);
...
- **Cerrado en ronda 4** (informe `DB-0-r4`, no versionado; los `.sql` 29–35 están en `supabase/migrations/`; migraciones 29–35): (1) **WhatsApp entrante desbloqueado** — `fn_update_customer_channel_identity` derivaba un `identity_type` invalido y usaba `external_message_id` como identidad, y el CHECK revertia el INSERT del mensaje; corregido el trigger (no el CHECK) y blindado con `EXCEPTION`. (2) **Cierre de oportunidades por `is_won`/`is_lost`** — `fn_sync_status_from_stage` redefinida y backfill de 14 etapas en 5 organizaciones (2, 120, 125, 130, 133); ya no se cierra una oportunidad ni se marca perdida por la probabilidad de la etapa. (3) Dos errores de tipo latentes en la cadena de comisiones (`payee_id` uuid) que habrian hecho fallar el cambio de etapa completo al cerrar el primer trato con comision. (4) `authenticated` sin EXECUTE sobre `decrement_ai_credits`/`deduct_comm_credits`/`refund_ai_credits`. (5) Advisors: `anon_security_definer` 335→332, `authenticated_security_definer` 336→333, `function_search_path_mutable` 437→436.
- **Archivos versionados en la ronda 3 de QA (2026-09-15, F0-DB r3)**: 55 migraciones `crm_v4_*` (35 `f00_01..35` + 20 de F04/F05/F06/F08/F09) + `crm_customer_lifecycle_ladder` = 56 `.sql` reconstruidos byte a byte desde `supabase_migrations.schema_migrations` (md5 verificado, cabecera de dos líneas) + 56 rollbacks en `supabase/rollbacks/`. Nota: el QA contó 58 porque sumó las dos `crm_*` de 2026-09-11/14 que ya tenían archivo. Migraciones nuevas de la ronda (pendientes de aplicar por el orquestador): `crm_v4_f00_36_ai_usage_logs_rls_y_rec_initplan` (usage logs sin INSERT para anon/authenticated, SELECT `TO authenticated` con `(select auth.uid())`; `call_recordings` rec_* con initplan), `crm_v4_f00_37_can_contact_guarda_y_release_job_tope` y `crm_v4_f00_limpieza_jobs_huerfanos` (6 jobs `crm_event` failed sin evento, org 125). Informe: `docs/crm-revenue-os/rondas/F0-DB-builder-r3.md`.
...
**Ronda 2 (2026-09-15) — respuesta al QA r1 (6/10). Informe: `docs/crm-revenue-os/rondas/F0-REG-builder-r2.md`**
- Migraciones escritas, PENDIENTES DE APLICAR (el orquestador las muestra al dueño): `supabase/migrations/20260915220000_crm_v4_f00_38_ai_settings_solo_rpc.sql` (saldo de IA solo por RPC/service role: revoke a anon, privilegios de columna a authenticated, políticas select/insert/update por pertenencia activa) y `supabase/migrations/20260915221000_crm_v4_f00_39_decrement_ai_credits_guardas.sql` (`decrement_ai_credits` con guardas NULL/negativo/sin fila → false; `refund_ai_credits(p_org_id, p_amount, p_previous)`; `fn_ai_usage_month(p_org, p_since, p_tz)` y `fn_comm_usage_month` agregando en SQL con `coalesce(cost_amount, metadata.cost_amount)`). Rollbacks en `supabase/rollbacks/` con el mismo timestamp.
- `aiCostService.ts`: `RangeError` ante importes no finitos o negativos antes del RPC; auto-provisión con `ensureAiSettings` (extraída de `checkAICredits`) y un reintento; `cost_amount` en columna (y en `metadata` una ronda más); reembolso idempotente por `logId` (memoria acotada + fila `:refund` en BD) con `p_previous`; `BudgetExceededError` (402, `code: 'budget_exceeded'`, subclase de `InsufficientCreditsError` para que `transcribe`/`analyze` la mapeen a `JobFatalError` sin tocar JOBS); `getMonthlyBudgetUsd` compartida con `GET /credits`.
- `aiCreditsService.ts`: `ensureAiSettings(orgId, client?)`; `consumeAICredits` `@deprecated` con guarda de importe; `withAICreditsCheck` `@deprecated` delegando en `withAiCharge` (misma firma; cobro ANTES del proveedor, 402 tipado). Guardarraíl #19 en `src/__tests__/guardrails.test.ts`: allow-list cerrada de los 11 llamadores V3 de `consumeAICredits`.
- `aiUsageStatsService.ts` (nuevo): `monthStartInTz`, `getAiUsageMonth`, `getCommUsageMonth` (RPC → respaldo en Node con `truncated` mientras la 39 no esté aplicada). `GET /api/crm/config/credits` los consume con `getOrgTimezoneServer`; sin `setUTCDate(1)` ni `.slice(0, 10)`.
- `pricingService.ts`: `valid_to` en `getUnitCost`/`listPricing` (misma regla que `fn_unit_cost`), `todayInTz('UTC')`, errores transitorios sin cachear.
- `providerCredentials.server.ts`: prioridad org > env entre proveedores (dos pasadas); `validateProviderSettings` contra `SETTING_FIELDS` (422; ≤ 4 KB; `null` borra); seed solo si `isOrgAdmin` (`GET /providers` de un miembro no escribe). `ProviderCard` envía solo las claves editables.
- `POST /providers/test`: con `source: 'env'` responde `{ ok, source, latencyMs }` sin detalle de la cuenta; `checkRateLimit`. `rbac.isOrgAdmin` por `roleId ∈ {1, 2}` o `isSuperAdmin`, sin nombres (ids verificados en `roles`). `isPlaceholderCredential('0000')` = placeholder.
- Tests: los 9 `it.failing` de `f0RegTester.r1.test.ts` y el `test.failing` de settings en `providerConfigContract.tester.test.ts` pasan a `it`/`test`; +6 casos en `aiCostService.test.ts` (auto-provisión, sin cupo, presupuesto ×2, `p_previous`, error genérico) y `crm/__tests__/aiUsageStats.test.ts` (8). Todo verde con `TZ=UTC` y `TZ=America/Bogota`.
- Fuera de REG, registrado en §11: `channel_credentials` en navegador (→ SEC) y rate-limit por instancia. `src/lib/utils/orgAdmin.ts` (`isOrgAdminLike`, GO Assistant) sigue resolviendo por nombre de rol: es de otra fase, se deja anotado.
...
- `orgContext.ts`: `resolveOrgFromExternal` con service client (C0); `getServerOrgContext(req?)` con header → cookie → `profiles.last_org_id` (consulta directa, tester #7) → única membresía → 400 `ORG_AMBIGUOUS`; `requireOrgAdmin` (criterio actual, F0-SEC C+D: `is_super_admin` o `role_id ∈ {1, 2}`; los nombres de rol dejaron de participar); `withOrg`/`withCron`.
...
## 13. Registro de implementación — ronda 3 (JOBS, 2026-09-15)

**Insumos:** `docs/crm-revenue-os/rondas/F0-JOBS-qa-r2.md` (7,5/10, puntos 1–6) y `F0-JOBS-tester-r2.md` (N-1…N-10). BD solo lectura por MCP. Informe: `docs/crm-revenue-os/rondas/F0-JOBS-builder-r3.md`.

**Decisión de scheduling (N-3).** Vercel Cron es el **primario**; pg_cron (17/18/19) el **respaldo**, apagado. Razones: (a) `9c0288a7` (2026-09-11, circuit-breaker por 522/544) bajó el drenaje total a `*/2 * * * *` sin tocar código, docs ni UI; (b) la evidencia en BD (jobs `recording_cleanup` creados a 08:30:28–29 todos los días desde el 09-11) prueba que Vercel ya ejecuta el cron diario; (c) pg_cron lleva una semana `active=false` y el 19 no pide `health_recalculate`/`renewals_sync`. El contrato vive en `src/lib/jobs/schedule.ts` (`DRAIN_INTERVAL_MIN = 2`, `DRAIN_SCHEDULE`, `VERCEL_SCHEDULE_KINDS`); lo consumen la ruta `run`, `JobsMonitor`, `jobsService.queuedOverdue` y el **guardarraíl 18** (`vercel.json` ↔ `schedule.ts`, sin cron strings ni «cada minuto» cableados en JOBS). Si el dueño enciende pg_cron, antes aplica `crm_v4_f00_41` (17 → `*/2`, 19 con los 4 kinds); hasta entonces no hay doble ejecución.

**Presupuesto real del cron diario (medido por el tester r2 en BD, 09-11…09-15):** dispara a 08:30:00; `maintenance` + `health_recalculate` + `renewals_sync` ≈ 29 s; encolar 84 orgs 7,4–10,2 s; último `done` a 08:30:51–56 ⇒ 52–57 s de `maxDuration = 60`. r3: el productor recibe `totalBudgetMs = 48 s` y cada paso como máximo lo que quede; el encolado pasa de 84 orgs a las que tienen grabaciones vencidas (hoy **1**: `call_recordings` solo tiene filas de una org) y devuelve `truncated`/`pending_org_ids` si se corta; la ruta no drena si quedan < 2 s (`reason:'budget_exhausted'`), y el `*/2` recoge lo encolado. Ya no existe `Math.max(2_000, …)`.

**N-4 (doble ejecución).** El `try` del handler y el de `fn_complete_job` están separados. Handler OK + RPC fallida ⇒ reintento único; si persiste, `fn_fail_job(-1)` ⇒ `failed` terminal con `last_error = 'complete_failed: …'` y `summary.completeFailed`. Se eligió `failed` terminal y no «dejarlo `running`» porque el reclaim de 10 min lo re-encolaría y el efecto se repetiría: `failed` es visible en JobsMonitor y solo un admin puede reintentarlo, y el reintento ya es idempotente. `whatsapp` fija `clientRequestId = job:{id}` (hereda `retried_from`) y salta si `findByClientRequestId` encuentra el mensaje (el contrato `{message_id}`+`pending` de §4.4 no es implementable: `messages` no tiene `status` ni `provider_message_id`). `whatsapp`/`email`/`transcribe` comprueban `signal.aborted` antes del efecto; el timeout del runner queda documentado como advisory.

**F-6.** `canViewJobs = canRetryJobs || roleId === 5`; `MANAGER_ROLE_NAMES` eliminado.

**N-7.** `src/lib/jobs/orgTimezone.ts` (`loadOrgTimezones`, `getOrgTimezoneForJob`, `orgDay`) sobre `organizations.timezone`; `recording_cleanup` corta por el día de la org; el productor usa el día de la org para clave y `scheduled_for`; `JobsTable` formatea con `useFormatDate().formatDateTime`.

**N-8 / N-9 / N-10.** Set de ids ejecutados por invocación (repetido ⇒ `fn_release_job` + fin del bucle); array ⇒ `p_result null`; `worker` del body solo `/^[A-Za-z0-9._:-]{1,100}$/`.

**Verificación (2026-09-15):** `TZ=UTC` y `TZ=America/Bogota` `npx jest src/lib/jobs` → 12 suites / **111 tests** verdes; `guardrails.test.ts` **71/71** (4 nuevos del caso 18); `f3Adversarial.test.ts` verde con el nuevo cutoff; `tsc` (heap 8 GB) sin errores en JOBS.

**Pendientes para el orquestador / dueño:** aplicar `crm_v4_f00_41` solo si se va a encender pg_cron; confirmar `CRON_SECRET` en Vercel; residuos N-5 (org 125, `entity_id` `bbbbbbbb-…`, 6 jobs `failed` del 09-10) — `maintenance` los purga el 2026-10-10, borrar antes si se prefiere; entrada «F0-JOBS r3» de `PROGRESS.md` (la escribe el orquestador).

## 13. Registro de implementación — ronda 4 (JOBS, 2026-09-15)

**Insumos:** `docs/crm-revenue-os/rondas/F0-JOBS-qa-r3.md` (8,2/10, puntos 1–5) y `F0-JOBS-tester-r3.md` (T-1…T-9). BD solo lectura por MCP (`information_schema.columns` y `pg_indexes` de `messages`, `email_messages`; conteo de filas). Informe: `docs/crm-revenue-os/rondas/F0-JOBS-builder-r4.md`.

**Decisiones registradas por el orquestador (PROGRESS.md, «escalado a decisión humana»).** (a) **Índice** sobre `messages.metadata->>'client_request_id'`: va como migración `crm_v4_f00_42` **pendiente de aplicar**, en el lote de migraciones que aplica el dueño. Como `apply_migration` corre en transacción, el `.sql` usa `CREATE INDEX IF NOT EXISTS` (SHARE lock: bloquea escrituras de `messages` mientras recorre 258 899 filas / 353 MB una vez; segundos) y la cabecera da la alternativa `CONCURRENTLY` a mano fuera de transacción. Predicado `(metadata->>'client_request_id') is not null` y no `metadata ? '…'`: el planificador solo usa un índice parcial cuyo predicado pueda demostrar desde el WHERE (`expr = $2` ⇒ `expr is not null`; el operador `?` no se deduce). Hoy 0 filas tienen la clave: el índice nace vacío. (b) **Permiso**: provisionalmente `admin.full_access` (`ORG_ADMIN_PERMISSION_CODE`, el mismo que `withOrg({admin:true})`) vía `hasOrgAdminOrPermission`; crear `crm.jobs.view` en `permissions` queda como decisión del dueño (sería una migración de seed + cambiar el código de permiso en `jobsService.ts`).

**T-2 (idempotencia fail-closed).** `findByClientRequestId` (F16, `outboundService.ts`) propaga `error` en vez de devolver `null`; el handler `whatsapp` lo convierte en `JobRetryableError('idempotency_check_failed: …')` sin enviar (backoff estándar); en `sendWhatsApp` sube como error interno (la ruta responde 500 `INTERNAL`; en campañas `classifySendError` ⇒ `retry`). `retryJob` hereda `retried_from` de la raíz (`payload.retried_from` si es string, si no `job.id`), así que la clave `job:{raíz}` es la misma en todas las generaciones. La ventana de 7 días (`CLIENT_REQUEST_ID_WINDOW_MS`) no se tocó: un retry manual de un `failed` de hace más de 7 días sigue pudiendo reenviar (documentado; ampliar la ventana es decisión de F16).

**T-1 (`signal` cooperativa, contrato honesto).** Ninguna de las tres funciones de efecto admite `signal`, así que no se tocaron F16/F7/F4. Los puntos de decisión son: `whatsapp` al entrar y tras `findByClientRequestId`; `email` al entrar y tras comprobar que `email_message_id` pertenece a `orgId` (comprobación nueva: `dispatchScheduledEmail` busca solo por id, y el payload nunca debe decidir la org; si no pertenece ⇒ `{skipped:true, reason:'not_found'}`, si la consulta falla ⇒ `JobRetryableError`); `transcribe` solo al entrar (la pertenencia, la transcripción existente y el cobro los hace `transcribeCall` en una secuencia). Un abort durante el efecto no lo cancela; lo cubre la idempotencia por estado. §4.4 lo dice así ahora.

**Permisos (punto 3).** `jobsService.ts` sin `MANAGER_ROLE_ID`: `canRetryJobs(ctx) = hasOrgAdminOrPermission(ctx)` y `canViewJobs(ctx) = isSuperAdmin || STAGE_MANAGER_ROLE_IDS.includes(roleId) || hasOrgAdminOrPermission(ctx)`; ambas `async`, las dos rutas usan `await`. La RPC recibe `p_user_id`/`p_organization_id` de la sesión; error ⇒ `false` + `console.warn`. Con un contexto sin sesión completa (tests antiguos) solo decide el criterio síncrono. El test del tester `testerR3.test.ts` «HUECO (regla 6)» se volteó: el nombre ya no concede (lo cerró F0-SEC r1 en `orgAdmin.ts`, que JOBS usa y no edita). **Hallazgo para F9, no para JOBS:** `STAGE_MANAGER_ROLE_NAMES` (`stagePermissions.ts:36-43`) sigue concediendo `canOverrideStageGate`/`canManageStages` por nombre de rol.

**T-6 / T-3 / T-4 / T-5.** Runner: `COMPLETE_RETRY_DELAY_MS = 300` entre los dos intentos de `fn_complete_job`, solo si `deadlineAt - now > 300` (la espera nunca se come el margen). Productor: `exhausted()` antes de la primera consulta (con presupuesto 0 o señal abortada ⇒ `{enqueued:0, orgs:0, truncated:true, reason:'budget_exhausted'}` sin round-trips) y otra vez tras seleccionar (⇒ `pending_org_ids` = todas, sin cargar zonas). F11: sin `Math.max(1_000, …)`; el presupuesto se recalcula antes de cada tarea y con `≤ 0` la tarea no arranca (`TaskFailure` con `reason:'budget_exhausted'`, log `warn`). `pending_org_ids`: decisión **no persistir** y retirar la promesa de `run/route.ts`, `scheduler.ts` y §4.4 (ver nota `:825`); el test del tester lo documenta como comportamiento aceptado.

**Verificación (2026-09-15):** `TZ=UTC` y `TZ=America/Bogota` `npx jest src/lib/jobs` → 15 suites / **166 tests** verdes (147 anteriores, con los `it.failing`/«HUECO» de T-1…T-4 ya como `it` normales, + 17 de `builderR4.test.ts` + 2 en `scheduler.test.ts`); `guardrails.test.ts` **76/76**; suites de F16/F7 (`src/lib/services/crm/{whatsapp,email}`) verdes tras el cambio en `findByClientRequestId`; `tsc` (heap 8 GB) **0 errores** en JOBS/`outboundService`/`jobsService` (los 8 restantes son preexistentes y ajenos).

**Pendientes para el orquestador / dueño:** aplicar `crm_v4_f00_42` (o el `CONCURRENTLY` a mano) fuera de horario comercial; decidir `crm.jobs.view`; `crm_v4_f00_41` solo si se enciende pg_cron; `CRON_SECRET` en Vercel; hallazgo F9 (`STAGE_MANAGER_ROLE_NAMES`); ventana de 7 días de F16; entrada «F0-JOBS r4» de `PROGRESS.md`.

## 13. Registro de implementación — ronda 5 (JOBS, 2026-09-16)

**Insumos:** `docs/crm-revenue-os/rondas/F0-JOBS-qa-r4.md` (9,1/10; obligatorios 1–3, opcionales 4–5) y `F0-JOBS-tester-r4.md` (N-2, N-3, M22, M26). BD ya aplicada por el orquestador antes de esta ronda: `f00_37` (`outbound_jobs.releases`), `f00_41` (pg_cron alineado), `f00_42` (índice `idx_messages_org_client_request_id`) y `f00_45` (`crm.jobs.view` y `crm.jobs.retry` en `permissions`, concedidos a los roles 1, 2 y 5). Sin migraciones nuevas en esta ronda. Informe: `docs/crm-revenue-os/rondas/F0-JOBS-builder-r5.md`.

**Decisión (b) del dueño y resolución del retry del Manager (QA r4 hallazgo 1, opción B).** Se mantiene la concesión de `crm.jobs.retry` al rol 5 que hace `f00_45` y **no hay `f00_46`**. Razón registrada por el orquestador con autorización del dueño: reintentar un job `dead|failed` es idempotente (reutiliza el `dedupe_key` y la clave `job:{raíz}` de `whatsapp`) y de bajo riesgo, y la jefatura comercial que ve la cola debe poder destrabarla; un cargo (`job_position_permissions`) puede negárselo con precedencia. El contrato inicial de r5 fue `retry ⇒ view` (quien puede reintentar puede ver); el **cierre r5** (QA r5 N-1, opción b, más abajo) lo sustituye por `canRetry = view ∧ retry`: reintentar exige ambos códigos y un cargo que niegue `view` cierra también el retry. La documentación r4 («Manager NO reintenta») quedó superada; §4.1 `:733-734`, la cabecera de `retry/route.ts` y el comentario de `jobsService.ts` lo dicen ahora.

**Permisos por código (obligatorio 2).** `jobsService.ts` ya no importa `STAGE_MANAGER_ROLE_IDS` ni conoce ninguna lista de roles: exporta `JOBS_VIEW_PERMISSION = 'crm.jobs.view'` y `JOBS_RETRY_PERMISSION = 'crm.jobs.retry'`; `canViewJobs(ctx) = hasOrgAdminOrPermission(ctx, 'crm.jobs.view')` y, desde el cierre r5, `canRetryJobs(ctx) = (await resolveJobsPermissions(ctx)).canRetry` (ya no consulta solo `crm.jobs.retry`: no puede contradecir a `resolveJobsPermissions`); toda RPC pasa por `hasJobsPermission`, envuelta en `try/catch` (una excepción del cliente, no solo `{ error }` de la RPC, también deniega y deja `console.warn`). `resolveJobsPermissions(ctx): Promise<{canView, canRetry}>` resuelve los dos permisos **una vez por petición** con la política (ii) del QA, secuencial y fail-closed: consulta `crm.jobs.view` primero; si es `false` ⇒ `{view:false, retry:false}` con **una sola RPC** (no se pregunta `retry`); si es `true` ⇒ consulta `crm.jobs.retry` (dos RPC, orden `[view, retry]`) y `canRetry = view ∧ retry`. `GET /api/crm/jobs` la llama una vez y usa `canView` para el 403 («Requiere permiso crm.jobs.view») y `canRetry` en la respuesta (N-2 cerrado); `POST …/retry` sigue con `readOrgBody` y luego `resolveJobsPermissions(ctx).canRetry` («Requiere permisos crm.jobs.view y crm.jobs.retry»), el mismo punto único. El rol 5 **nunca** entra por el criterio síncrono: siempre consulta la BD, así que un cargo que le niegue `crm.jobs.view` lo deja fuera aunque sea Manager, y también le impide reintentar aunque el rol le conceda `crm.jobs.retry` (QA r5 N-1). A la inversa (QA r5 hallazgo 3): un cargo que concede **solo** `crm.jobs.retry` a un rol sin `crm.jobs.view` no concede nada —para conceder el retry por cargo hay que conceder también la vista—; en la BD no había ninguna fila de `job_position_permissions` con `crm.jobs.%` al cerrar r5, así que nadie pierde acceso real. Los roles 1/2 y el super admin siguen entrando por `isOrgAdminLike` sin RPC: **un cargo no puede negar la cola a un administrador** (coherente con `withOrg({admin:true})`; QA r4 hallazgo 4, documentado y no cambiado). Sin sesión completa (`userId`/`organizationId`/`supabase`) no hay RPC y solo decide el criterio síncrono.

**Tests volteados/añadidos (r5; los conteos y el orden de RPC de este párrafo son los de la política (i) y quedaron re-volteados en el cierre r5, ver más abajo).** `jobsService.test.ts` (Manager con sesión ve y reintenta por la RPC; cargo que niega ⇒ fuera; `resolveJobsPermissions` fail-closed ante `error` y ante excepción), `builderR4.test.ts` r4·3 (sin `STAGE_MANAGER_ROLE_IDS`; la RPC recibe `crm.jobs.*` y nunca `admin.full_access`; test estático: `jobsService.ts` sin listas ni `roleId === N` ni `roleName`, la GET con **una** `resolveJobsPermissions(ctx)`), `testerR3.test.ts` (sin sesión completa coincide con `ORG_ADMIN_ROLE_IDS`, no con una lista propia; Manager reintenta con `f00_45`), `testerR4.test.ts` (rutas: con solo `crm.jobs.view` ⇒ 200 `canRetry:false` y retry 403; Manager con los dos códigos ⇒ 200 y retry llega a `retryJob`; Manager con cargo que niega ⇒ 403 en ambas; «GET con cargo» ya es aserción: ≤ 2 RPC). Nuevo `builderR5.test.ts`: **contrato BD ↔ código ↔ doc** (lee `f00_45`: los dos códigos de `jobsService.ts` concedidos a `[1, 2, 5]`, rollback presente, sin credenciales; ningún `.sql` posterior retira `(5, crm.jobs.retry)`; `ORG_ADMIN_ROLE_IDS = [1, 2]`; §4.1 `:733-734` y la cabecera de retry sin `admin.full_access`/`STAGE_MANAGER_ROLE_IDS`/«Manager NO»), permisos por rol de sistema con los grants de `f00_45`, y las dos guardas de un solo test (opcional 4): `readOrgBody` en retry con **super admin** y body/query de otra org ⇒ 403 sin `getServiceClient` ni RPC (2.ª guarda de M26); T-6 con `expect(COMPLETE_RETRY_DELAY_MS).toBe(300)`, el literal en `runner.ts` y Δt ≥ 295 ms medido sin restar la constante (2.ª guarda de M22).

**N-3 (opcional 5).** `scheduler.ts`: con el total ya agotado (`remainingFor(budgetMs) <= 0`), `maintenance` no arranca ⇒ `{ ok:false, ms:0, error:'budget_exhausted' }` + `log.warn('maintenance_skipped')`, sin `runMaintenance` ni los 250 ms de timer mínimo; con presupuesto positivo el mínimo de 250 ms se conserva. El test «RESIDUO» de `testerR4` pasó a aserción y hay uno nuevo para el caso positivo por debajo de 250 ms.

**No tocado (por instrucción):** `src/__tests__/guardrails.test.ts`, `src/lib/security/**`, `src/lib/utils/orgContext.ts` (otro builder en curso). `retry/route.ts` solo cambió cabecera y mensaje de 403.

**Cierre r5 (2026-09-16, QA r5 N-1, opción b — fail-closed).** El tester r5 demostró con un dry-run real (`rollback`) que `check_user_permission` resuelve un código por llamada y que un cargo que niega **solo** `crm.jobs.view` a un Manager devuelve `view=false, retry=true`; con la política (i) el código nunca consultaba `view` y respondía `GET 200` y retry permitido (deny explícito ignorado). Decisión del orquestador: **(b)** `canRetry = view ∧ retry`. Cambios: `resolveJobsPermissions` consulta `crm.jobs.view` **primero** y corta en `false` (`{canView:false, canRetry:false}`, 1 RPC); solo con `view=true` consulta `crm.jobs.retry` (2 RPC, orden `[view, retry]`). `canRetryJobs(ctx)` delega en `resolveJobsPermissions` (punto único, regla 7) y `POST …/retry` usa `(await resolveJobsPermissions(ctx)).canRetry` con 403 «Requiere permisos crm.jobs.view y crm.jobs.retry». Coste de RPC (N-2 se reabre parcialmente y se acepta): el Manager por defecto paga 2 RPC en la GET y 2 en el retry (antes 1 y 1); quien no ve paga 1 (antes 2); `JobsMonitor` refresca cada 2 min, coste asumido como precio del fail-closed; `Promise.all` no reduce el conteo y pierde el cortocircuito, por eso la política es secuencial. Cambio de semántica visible (QA r5 hallazgo 3): un cargo que concede solo `crm.jobs.retry` a un rol sin `view` pasa de 200/404 a 403/403 —para conceder el retry por cargo hay que conceder también la vista—; sin filas `job_position_permissions` con `crm.jobs.%` en la BD, nadie pierde acceso. Tests: los 24 rojos del hallazgo 4 del QA r5 volteados (no borrados) en `jobsService.test.ts`, `builderR4.test.ts`, `builderR5.test.ts`, `testerR4.test.ts` y `testerR5.test.ts` (el caso N-1 pasa de documentar el defecto a demostrar la corrección; `testerR5` «N-1 documental» se conserva porque la frase de este registro ahora es verdad); nuevo `builderR5b.test.ts` con los 10 puntos del QA r5 y las mutaciones que deben morir (orden invertido, sin cortocircuito, `canRetry` sin `∧ view`, `canRetryJobs` consultando solo `retry`, retry/route.ts llamando a `hasOrgAdminOrPermission` directo). Informe: `docs/crm-revenue-os/rondas/F0-JOBS-builder-r5b.md`.

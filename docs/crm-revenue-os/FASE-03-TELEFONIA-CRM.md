...
│     (r5: SIN acta al marcar; la escribe consent-whisper al contestar) → TwiML     │
...
**`POST /api/voice/twiml/outbound`** (client-originated). Payload real (form-urlencoded): `AccountSid=AC…&ApiVersion=2010-04-01&ApplicationSid=AP…&CallSid=CA…&CallStatus=ringing&Caller=client:u_1f2e…_o_7&Direction=inbound&From=client:u_1f2e…_o_7&To=+573001234567&opportunityId=…&customerId=…`. Nota: para el TwiML App, `Direction=inbound` (la llamada "entra" desde el cliente WebRTC) y el `CallToken` viene en el primer webhook. Verificación: firma con token resuelto por `AccountSid`. Mapeo: identity → `{userId, orgId}`; comprobar que `userId` es miembro activo de `orgId` (`organization_members`) y `accountSidMatchesOrg`; `To` → E.164 (`libphonenumber-js`); insert `calls` (sin acta: ronda 5). Con fila ya existente (reintento de Twilio) `record=` sale de `calls.recording_enabled` de esa fila (ronda 6, N-3). La rama REST (`From` = número, creada por `/api/voice/call`) está **cerrada** con `<Hangup/>` salvo `VOICE_LEGACY_REST_OUTBOUND=true` (ronda 6, N-5).
...
**`POST /api/voice/recording`**: `AccountSid, CallSid, RecordingSid, RecordingUrl (sin extensión), RecordingStatus (in-progress|completed|absent), RecordingDuration, RecordingChannels (1|2), RecordingSource (DialVerb|RecordVerb|…), RecordingStartTime, RecordingTrack (both)`. Suscribimos `in-progress completed absent` (ronda 6). `completed` → upsert `call_recordings` {provider_recording_sid, channels: String(RecordingChannels), duration_seconds, storage_path: buildStoragePath(orgId, callId), storage_provider:'twilio', status:'processing'}` y encola `outbound_jobs(kind:'recording_fetch', dedupe recording_fetch:{RecordingSid}, max_attempts:6)`; si la llamada **no tiene acta** (el whisper que Twilio no consiguió traer), queda un acta `method='unverified_announcement'` con `calls.consent_given=false` y `metadata.consent_unverified_reason` (ronda 6, punto a). `in-progress` → `calls.metadata.recording_started_at` (evidencia de que arrancó). `absent` → si la llamada no tiene ninguna grabación registrada, `voidConsentWithoutRecording` retira el acta y deja `recording_enabled=false`, `consent_given=false`, `metadata.recording_absent_at` (ronda 5). El simétrico sin callback alguno lo cierra la reconciliación diaria `consentReconcileService` dentro de `runMaintenance` (ronda 6, punto b).
...
- `POST /api/voice/twiml/outbound` reescrito con dos ramas: **A** client-originated (`From=client:u_…_o_…`) que resuelve la org por la identity, valida membresía, comprueba que el `AccountSid` corresponde a la (sub)cuenta de la org, elige caller id, **reserva 1 minuto de créditos** e inserta la fila `calls` (`mode='browser'`, `status='dialing'`) + `call_consents` (**superado en la ronda 5: ya no inserta `call_consents`**, lo hace `consent-whisper` al contestar); **B** REST-originated (F5/F6) que resuelve por `CallSid` (**cerrada por defecto desde la ronda 6**). Idempotente por `CallSid`.
...
## 15. Registro de implementación — rondas 3 a 6 (2026-09-10 → 2026-09-14)

Resumen; el detalle ronda a ronda vive en `docs/crm-revenue-os/PROGRESS.md` («Zona de voz»).

### Ronda 3 (2026-09-10) — tester 8,5
- Cerrados N-1, N-2, N-3, N-4 y N-6 de la ronda 2 con 17 reversiones en rojo. **N-4 de raíz**: el aviso de grabación de `inbound` viaja en su propio documento TwiML (`<Say>` + `<Redirect>`), la llamada nace con `consent_given=false` y solo la segunda pasada marca el consentimiento y devuelve el `<Dial record=>`.
- Hallazgo del tester (F-1): la marca de «aviso reproducido» era `?announced=1` en la query, que controla quien firma: acta fabricable de un tirón.

### Ronda 4 (2026-09-10) — construida junto con F5 r2 por UN solo constructor de zona de voz
- A-1: la prueba de que el aviso sonó es un HMAC ligado al `CallSid` con caducidad (`signConsentToken`/`verifyConsentToken`, el mismo primitivo que los tokens del bridge). Sin `VOICE_CALLBACK_SECRET` no se puede acreditar: no se graba, la llamada se atiende.
- A-3/M1: `accountSidMatchesOrg` en las 11 rutas de voz; con 0 de 83 organizaciones con subcuenta, la comprobación cae al SID maestro y falla cerrado.
- A-6: índice único `call_consents_org_call_type_uidx (organization_id, call_id, consent_type)` aplicado por MCP (tabla con 0 filas entonces).
- Incidente: una mutación de prueba (`if (false && …)` en `bridgeTwimlBuilders.ts`) llegó a HEAD y el puente grababa sin aviso; la red mordió (F5-38/39/40/55) y la restauración falló. Corregido con comentario en el sitio.

### Ronda 5 (2026-09-14) — «nunca grabar sin acta», en UN solo sitio — tester F3 8,5 · F5 7,5
- Escrituras a mano de `call_consents`: 4 → 0. Único escritor `consentService.recordConsent` (`upsert` sobre el índice único con `ignoreDuplicates`, `announced_at` explícito, lanza si la marca en `calls` no alcanza fila de la org). La prueba G.1 recorre `src/`.
- V-1…V-5: `outbound` no escribe acta al marcar (evidencia en producción: 4 de 6 actas eran de llamadas nunca contestadas); `consent-whisper` escribe el acta al servir el aviso y falla cerrado con `<Hangup/>`; `inbound` y `ai-agent` sin acta no emiten `record=` y dejan la fila en `false`; `ai-agent` graba con `<Start><Recording>` (vía documentada por Twilio para ConversationRelay), nunca antes del acta; `/api/voice/recording` trata `absent` (`voidConsentWithoutRecording`).
- 33 pruebas (29 rojas → verdes), 19 mutaciones muertas.

### Ronda 6 (2026-09-14) — una sola fuente de verdad y el simétrico «nunca acta sin grabación»
- **N-1 (F5)**: `customer-leg`/`agent-leg` deciden `record=` por `calls.recording_enabled` (`recordingEnabledForCall`, fallo cerrado), la MISMA fila que lee el whisper; ya no releen `comm_settings` al conectar. Encender la grabación entre marcar y conectar ya no produce `<Dial record=>` con whisper vacío.
- **N-2**: `ai-agent` retira el acta (`voidConsentWithoutRecording`) si algo falla después de escribirla y antes de devolver el TwiML (Twilio nunca enviaría `absent` de una grabación que no arrancó).
- **N-3**: prueba que muerde para la rama de idempotencia de `outbound` (`recordingEnabled: ex.recording_enabled`).
- **N-4**: `manualCallService` escribe el acta por `recordConsent` con `method='manual'` (columna sin CHECK, verificado por MCP) y la evidencia (usuario, fecha, origen) ANTES de subir el audio; sin acta no se guarda la grabación. G.4 recorre TODO `src/`.
- **N-5**: rama REST de `outbound` cerrada con `<Hangup/>` salvo `VOICE_LEGACY_REST_OUTBOUND=true`; con bandera, `accountSidMatchesOrg` obligatorio.
- **(b)** `recordingStatusCallbackEvent="in-progress completed absent"`; `in-progress` deja `metadata.recording_started_at`. Reconciliación diaria `consentReconcileService.reconcileConsentsWithoutRecording` (paso 6 de `runMaintenance`, cron de F0): llamadas terminadas hace más de 10 min y menos de 7 d con `recording_enabled`, `consent_given` y sin `call_recordings`: pregunta a Twilio por REST (`recordings.list`, solo lectura); si la grabación existe la registra y encola `recording_fetch` (el callback se perdió); si Twilio responde que no existe (evidencia positiva, r7) retira el acta; si Twilio no responde o no hay credenciales, difiere y **la llamada sigue siendo candidata aunque salga de la ventana de 7 días** (marca `calls.metadata.consent_reconcile_deferred_at`/`consent_reconcile_deferrals`, r8). Aislado del resto del mantenimiento.
- **(a)** `completed` sin acta: se registra la grabación y queda un acta `unverified_announcement` con `consent_given=false` y `metadata.consent_unverified_reason`, más error en el log.
- **N-6**: `inbound` con fila ya creada respeta `recording_enabled` de la fila en el reintento. **N-7**: `ai-agent` saca el origen de los callbacks de `getTwilioWebhookOrigin()`. **N-9**: `agentRuntime` sin fila `comm_settings` no graba (`=== true`, como `getTelephonySettings`).
- Suite `f3f5Round6Consent` (30 casos, 20 rojos → verdes), 25 mutaciones muertas y restauradas por md5. Contratos cambiados a propósito: F5-41 (la fila manda), F5-42 y twimlBuilders (`in-progress`), V-1.2…V-1.5 (bandera REST), A-1.6 (el reintento respeta la fila en `false`).
- Migración de datos **preparada y NO aplicada** (`supabase/migrations/20260914150000_voz_r6_retirar_actas_llamadas_no_contestadas.sql` + rollback con las 4 filas): retira las 4 actas de llamadas no contestadas de la org 125. La quinta acta (contestada, 4 s, sin grabación) la cubrirá la reconciliación diaria cuando se despliegue.


### Ronda 7 (2026-09-15) — tester r6 → F3 9,2 · F5 9,5 (F5 aprobada)
- **F-1a/F-1b**: la reconciliación retira el acta **solo con evidencia positiva de ausencia** (`absent`, lista vacía, sin `provider_call_sid`); sin credenciales (`VoiceNotConfiguredError`) o grabación en `processing`/`paused` → `deferred`; `completed` → `recovered`. Antes, una organización sin credenciales perdía todas sus actas candidatas.
- **F-2**: el agente IA decide la grabación por UNA sola fuente (`recordingEnabledForCall` sobre la fila `calls`) también en `agentRuntime`; `voiceAgentService` fija la fila con `=== true`. **F-3**: `inbound` sin agentes no escribe acta ni toca una preexistente.
- **F-4**: `consent_method` viaja en `CallListRow`/`TimelineCallData` (embed `call_consents(consent_type, method)`, FK única confirmada por MCP); distintivo «Aviso no acreditado» (`ConsentBadge.tsx`, icono + texto + `title`, `role="status"`); la acta no acreditada lo dice en su propio texto y `announced_at` lleva su nota en `metadata`.
- **F-5**: `recording_declaration` obligatoria en `/api/crm/calls/manual` y `/api/crm/transcribe` (400 `RECORDING_DECLARATION_REQUIRED` con el texto legal); el acta ya no inventa «El usuario declara…». **F-6**: `POST /api/voice/call` → 410 salvo `VOICE_LEGACY_REST_OUTBOUND=true`.
- Tester r7 (`f3f5Round7Tester`, 24 casos): tabla de invariantes por ruta completa (reintentos idempotentes, `absent` tardío no retira, `completed` sin acta → `unverified`). Hallazgos que dejaron F3 en 9,2: H-1 (la ventana de 7 d + `deferred` = acta huérfana permanente), H-2 (`agentRuntime` sin fila enlazada caía a `comm_settings`), H-3 (`recordingDeclaration: true` cableado en las rutas), H-4 (F-4 guardado solo por cadenas), H-5 (rondas sin registrar aquí).

### Ronda 8 (2026-09-15) — cierre de F3: H-1…H-5
- **H-1**: `consentReconcileService` hace dos consultas: dentro de la ventana (marcadas o no) y, fuera de ella, **solo las ya diferidas** (`metadata->>consent_reconcile_deferred_at IS NOT NULL`). Cada `deferred` persiste `consent_reconcile_deferred_at` y `consent_reconcile_deferrals` (fusión de `metadata`, sin pisar disposición ni costes); a partir de 20 diferimientos o 30 días desde `ended_at` se registra `consent_reconcile_stale_candidate` (contador `stale` en el resultado) **sin retirar el acta**: solo la evidencia positiva la retira. `recovered` retira la marca (`consent_reconcile_resolved_at`) para que la fila no vuelva a entrar cada día. Caso real cubierto: la acta de 16 s de la org 125 (terminada el 2026-09-13 03:25 UTC) sigue siendo candidata después del 2026-09-20.
- **H-2**: `agentRuntime` sin fila `calls` enlazada → `recordingEnabled=false` (la ruta no graba sin fila; el prompt no puede decir que graba). N-9.2 de `f3f5Round6Consent` cambia de contrato (lo documenta en el propio test); F2.6 del tester pasa a verde sin `.failing`.
- **H-3**: las dos rutas pasan al servicio `readRecordingDeclaration(form)` leído (`const recordingDeclaration = …`), nunca `true` cableado; D.5 es ahora de conducta con el servicio REAL (sin casilla → 400, cero filas, cero subidas; con casilla → acta `manual` con «Declaración marcada por el usuario»).
- **H-4**: `isUnverifiedConsent` vive en `src/lib/services/crm/consentMethod.ts` (puro, importa `UNVERIFIED_CONSENT_METHOD` de `consentService`; `ConsentBadge.tsx` lo reexporta) y `listCallsWithRelations` se prueba con un fake que devuelve el embed: M20 y M21 mueren.
- **Nota 6**: `callAnalysisService` importa `stageGateService` (que crea el cliente de NAVEGADOR al cargarse) bajo demanda en `applyAnalysis`; prueba N6.1 con `@/lib/supabase/config` reventando al importar.
- Suite `f3f5Round8Builder` (15 casos, 8 rojos → verdes, reloj controlado); 37 mutaciones (las 23 del tester r7 + 14 propias) muertas y restauradas por md5. Pendiente ajeno a F3: `cron.job` 17/18/19 siguen `active=false` (depende del cron de Vercel); la migración de datos de la ronda 6 sigue preparada y NO aplicada.

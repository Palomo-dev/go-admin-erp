...
| F2 Terminal, ajustes, propina, QR | **Parte A (terminales y ajustes completos) en ronda 2.** `pos_terminals` aplicada (`20260916010000`) y endurecida (`20260921140000`: insert/update exigen `branch_id` de la misma organización; sin política DELETE). Servicio `src/lib/services/posTerminalsService.ts` (listar/crear con RLS; renombrar y activar/desactivar por `PATCH /api/pos/terminals/[id]` con rol admin/manager en servidor; identidad pura en `src/lib/pos/display/terminalIdentity.ts`), esquema zod completo de §5.2 en `settings.ts` (presets: tres enteros distintos 1–100, ordenados), tarjeta con `AjustesPantallaSection` + `EstaCajaSection` (vincular, crear, renombrar, activar; un solo `saving` compartido con el interruptor), `hello.settings` aditivo en el protocolo, firma de líneas en `setTotals` (incluye `taxExcluded`/`taxIncluded`). Partes B (propina) y C (QR) pendientes. | `workflow-f2.js`; `npx jest src/__tests__/pos-display` |

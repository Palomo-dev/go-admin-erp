export const meta = {
  name: 'pos-doble-pantalla-f0-v2',
  description: 'Fase 0 v2: ronda de cierre de la Parte A con el feedback acumulado, luego B/C/D en paralelo e integración',
  phases: [
...
Feedback acumulado de las rondas 1-3 (el codigo ya paso por tres rondas: 11 suites / 179 tests en verde y eslint limpio; NO reescribas, cierra):
1. [medio · tester r3] projectCartForDisplay lanza TypeError si items o modifiers traen null (carritos viejos de localStorage): filtrar entradas nulas antes de proyectar y cubrirlo con test.
2. [medio · tester r3] BroadcastChannelReceiver no puede soltar la instancia activa si la pestaña muere sin bye: liberarla al superar 3 s sin heartbeat (o exponer releaseActiveInstance() y que la logica de Conectando la llame), para que el siguiente need_snapshot no vaya a una instancia muerta. Test con fake timers.
3. [medio · tester r3] Carrera con dos pestañas de /app/pos al abrir la pantalla: el need_snapshot inicial va sin destinatario y "gana el ultimo hello". Definir la regla de adopcion (recomendada: preferir el hello con sessionOpen=true; a igualdad, el de seq mayor) e implementarla y documentarla en el JSDoc del receptor. Test con dos emisores.
4. [bajo · tester r3] projectLine copia item.id sin normalizar: si falta, generar un id estable derivado (p. ej. product_id + indice) para que la UI tenga key.
5. [bajo · tester r3] Un bye de instancia desconocida sin instancia activa se adopta y libera en el mismo mensaje y llega a la UI: ignorarlo sin entregarlo.
6. [bajo · qa r2] JSDoc de DisplayLine.modifiers[].extraPrice: es informativo, ya esta dentro de unitPrice.
7. [bajo · qa r2] Exponer en el receptor algo que distinga bye de silencio (p. ej. lastByeAt) y deduplicar un hello repetido con el mismo seq.
8. [bajo] El PLAN.md §8 ya fue actualizado por el orquestador con instanceId/toInstanceId/DownMessageDraft/heartbeat: verifica que el JSDoc de protocol.ts y transport.ts diga lo mismo.
`

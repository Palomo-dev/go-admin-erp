// ============================================================
// GET /api/integrations/redeban/status
// Consulta el estado de una sesion QR (usado por QrPoller).
// ============================================================
...
export async function GET(request: NextRequest) {
  try {
    ...
    // 2. Si sigue pendiente, consultar estado a Redeban (opcional)
    if (qrSession.status === 'pending' && qrSession.integration_connection_id && qrSession.external_qr_id) {
      try {
        ...
        console.error('[API Redeban Status] Error consultando proveedor:', pollErr);
      }
    }
    ...
    console.error('[API Redeban Status] Error:', err);
    return NextResponse.json(

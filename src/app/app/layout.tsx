...
import {
  getOptimizedSession,
  isAuthenticated,
} from "@/lib/supabase/auth-manager";
      ...
      >
        <Link
          ...
          className={`flex items-center ${
            collapsed ? "lg:justify-center lg:w-full" : "flex-grow"
          }`}
        >
          <span
            className={`
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex justify-between items-center p-4 bg-blue-600">
            ...
            {sidebarCollapsed && (
              <h1 className="hidden lg:block text-xl font-bold text-white text-center">
                GO
              </h1>
            )}
          ...
          {orgName && (
            <div
              className={`m-3 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg shadow-sm border border-blue-100 dark:border-blue-800 ${
                sidebarCollapsed ? "lg:relative lg:group" : ""
              }`}
            >
              <div className="relative">
                <p className="text-sm font-medium text-blue-900 dark:text-blue-100 flex items-center">
                  <Building2
                    size={16}
                    className={`${sidebarCollapsed ? "lg:mx-auto" : "mr-2"}`}
                  />
                  {!sidebarCollapsed && orgName}
                  {sidebarCollapsed && (
                    <span className="lg:hidden">{orgName}</span>
                  )}
                </p>
        ...
        <div className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
          <div className="flex justify-between items-center px-4 py-2">
            ...
            <div className="flex items-center space-x-3">
              {/* Branch Selector */}
              {orgId && <BranchSelector organizationId={parseInt(orgId)} />}
  ...
  return (
    <div className="space-y-1 transition-all duration-300">
      ...
      <div className="pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
        ...
        <Link
          href="/app/perfil"
          className={`block px-3 py-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors rounded-md ${
            collapsed ? "lg:px-2 lg:text-center lg:relative lg:group" : ""
          }`}
        >
          <div
            className={`flex ${

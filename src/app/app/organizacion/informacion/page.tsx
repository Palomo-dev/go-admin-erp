  ...
  if (loading) {
    return (
      <div className={`p-4 sm:p-6 space-y-6 bg-gray-50 min-h-screen ${resolvedTheme === 'dark' ? "bg-gray-900" : ""}`}>
        <div>
          <h1 className={`text-2xl font-semibold text-gray-900 ${resolvedTheme === 'dark' ? "text-gray-100" : ""}`}>{t('info.title')}</h1>
          <p className={`mt-2 text-gray-600 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>{t('info.description')}</p>
        </div>

...
export function GymConfigPanel() {
  ...
  const updateAccessRules = (key: keyof GymSettings['accessRules'], value: boolean | number) => {
    setSettings((prev) => ({ ...prev, accessRules: { ...prev.accessRules, [key]: value } }));
  };
  const updateTolerance = (key: keyof GymSettings['tolerance'], value: number) => {
    setSettings((prev) => ({ ...prev, tolerance: { ...prev.tolerance, [key]: value } }));
  };
  const updateCheckinMethods = (key: keyof GymSettings['checkinMethods'], value: boolean) => {
    setSettings((prev) => ({ ...prev, checkinMethods: { ...prev.checkinMethods, [key]: value } }));
  };
  const updateMessages = (key: keyof GymSettings['messages'], value: string) => {
    setSettings((prev) => ({ ...prev, messages: { ...prev.messages, [key]: value } }));
  };
  const updateClassRules = (key: keyof GymSettings['classRules'], value: boolean | number) => {
    setSettings((prev) => ({ ...prev, classRules: { ...prev.classRules, [key]: value } }));
  };
      ...
      <AlertDialog open={showResetDialog} onOpenChange={setShowResetDialog}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={confirmReset} className="bg-red-600 hover:bg-red-700">Restablecer</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>

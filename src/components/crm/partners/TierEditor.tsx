...
export function TierEditor({ open, tiers, onOpenChange, onSave, onDelete, returnFocusFallback }: Props) {
            ...
            {tiers.map((t) => {
              ...
              return (
                <li key={t.id} className={cn('rounded-xl border bg-white p-4 dark:bg-gray-900', isEditing ? 'border-blue-300 dark:border-blue-800' : 'border-gray-200 dark:border-gray-800')}>
                  ...
                  {isEditing && <div className="mt-4 border-t border-gray-100 pt-4 dark:border-gray-800"><TierForm tier={t} onSave={onSave} onCancel={() => setEditingId(null)} /></div>}
                </li>
              );
            })}
          </ol>

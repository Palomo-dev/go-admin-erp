...
export interface JobPositionEditFormData {
  name: string;
  code: string;
  description: string;
...
interface JobPositionEditFormProps {
  position: JobPositionData;
  onSubmit: (data: JobPositionEditFormData) => Promise<void>;
  onCancel: () => void;
  departments: DepartmentOption[];
  levels: string[];
  isLoading?: boolean;
  validateCode?: (code: string, currentId: string) => Promise<boolean>;
}

const DEFAULT_LEVELS = ['Junior', 'Mid', 'Senior', 'Lead', 'Manager', 'Director', 'C-Level'];

export function JobPositionEditForm({
  position,
  onSubmit,
  onCancel,
  departments,
  ...
  const [formData, setFormData] = useState<JobPositionEditFormData>({
    name: position.name,
    code: position.code || '',
  ...
  useEffect(() => {
    const changed =
      ...
      formData.code !== (position.code || '') ||
      formData.description !== (position.description || '') ||
      formData.department_id !== position.department_id ||
      formData.level !== (position.level || '') ||
      formData.min_salary !== position.min_salary ||
      formData.max_salary !== position.max_salary ||
      formData.is_active !== position.is_active;
    setHasChanges(changed);
  }, [formData, position]);

  // Validar código único con debounce (solo si cambió)
  useEffect(() => {
    const originalCode = position.code || '';
    if (!formData.code || formData.code.length < 2 || formData.code === originalCode) {
      setCodeStatus('idle');
      return;
    }
    ...
    const timer = setTimeout(async () => {
      if (validateCode) {
        ...
        try {
          const isUnique = await validateCode(formData.code, position.id);
  ...
  }, [formData.code, validateCode, position.code, position.id]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(error.message || 'Error al actualizar el cargo');
    } finally {
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              El cargo &quot;{formData.name}&quot; se ha actualizado correctamente.
            </p>
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-center justify-between pt-4">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button
            type="submit"
            disabled={isSubmitting || isLoading || codeStatus === 'checking' || !hasChanges}
...
export default JobPositionEditForm;

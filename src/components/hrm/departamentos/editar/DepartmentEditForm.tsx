...
interface DepartmentEditFormProps {
  ...
  onSubmit: (data: DepartmentEditFormData) => Promise<void>;
  onCancel: () => void;
...
export function DepartmentEditForm({
  ...
  const [formData, setFormData] = useState<DepartmentEditFormData>({
  ...
  useEffect(() => {
    const changed =
      ...
      formData.manager_employment_id !== department.manager_employment_id ||
      formData.cost_center !== (department.cost_center || '') ||
      formData.is_active !== department.is_active;
    setHasChanges(changed);
  }, [formData, department]);

  // Validar código único con debounce (solo si cambió)
  useEffect(() => {
    const originalCode = department.code || '';
    ...
    const timer = setTimeout(async () => {
      if (validateCode) {
        ...
        try {
          const isUnique = await validateCode(formData.code, department.id);
  ...
  }, [formData.code, validateCode, department.code, department.id]);
  ...
  const validate = (): boolean => {
    ...
    if (formData.parent_id === department.id) {
      newErrors.parent_id = 'Un departamento no puede ser su propio padre';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitError(error.message || 'Error al actualizar el departamento');
    } finally {
  ...
  // Filtrar departamentos para no incluir el actual ni sus hijos (evitar ciclos)
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col items-center justify-center py-8 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              El departamento &quot;{formData.name}&quot; se ha actualizado correctamente.
            </p>

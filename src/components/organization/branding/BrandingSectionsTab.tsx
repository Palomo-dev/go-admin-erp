...
export default function BrandingSectionsTab({ settings, onSave, isSaving }: BrandingSectionsTabProps) {
  ...
  return (
    <div className="space-y-6">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <Label className={`font-medium ${themeClass("", "text-white")}`}>{t(section.labelKey)}</Label>
                      <p className={`text-xs text-gray-500 ${themeClass("", "text-gray-400")}`}>{t(section.descKey)}</p>
                    </div>
                  </div>
                  <Switch
                    checked={isEnabled}
      ...
      <Card className={themeClass("", "bg-gray-800 border-gray-700")}>
        <CardHeader>
          <CardTitle className={themeClass("", "text-white")}>{t('orderTitle')}</CardTitle>
          <CardDescription className={themeClass("", "text-gray-400")}>
            {t('orderDesc')}
          </CardDescription>
        </CardHeader>
          ...
          <div className="space-y-2">
            {SECTIONS.filter(s => formData[s.key as keyof typeof formData]).map((section, index) => {
              ...
              return (
                <div
                  key={section.key}
                  className={`flex items-center gap-3 p-3 bg-gray-50 rounded-lg ${themeClass("", "bg-gray-700/50")}`}
                >
                  <span className="w-6 h-6 flex items-center justify-center bg-blue-600 text-white text-xs font-medium rounded-full">
                    {index + 1}
                  </span>
                  <Icon className={`h-4 w-4 text-gray-500 ${themeClass("", "text-gray-400")}`} />
                  <span className={themeClass("", "text-white")}>{t(section.labelKey)}</span>
                </div>
              );
            })}
            {enabledCount === 0 && (
              <p className={`text-center text-gray-500 py-4 ${themeClass("", "text-gray-400")}`}>
                {t('noActiveSections')}
              </p>
            )}
          </div>

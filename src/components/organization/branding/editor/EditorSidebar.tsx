...
export default function EditorSidebar({
  ...
  return (
    <div className={`w-[320px] min-w-[320px] bg-white text-gray-800 flex flex-col h-full border-r border-gray-200 ${resolvedTheme === 'dark' ? "bg-gray-900 text-white border-gray-700" : ""}`}>
      {/* Sidebar Header */}
      <div className={`p-3 border-b border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
        <h2 className={`text-sm font-semibold text-gray-500 uppercase tracking-wider ${resolvedTheme === 'dark' ? "text-gray-300" : ""}`}>
          {t('sections')}
        </h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className={`border-b border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
          <button
          ...
          >
            <Settings className={`h-4 w-4 text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`} />
            <span className={`flex-1 text-left font-medium text-gray-700 ${resolvedTheme === 'dark' ? "text-gray-200" : ""}`}>{t('themeConfig')}</span>
            {showGlobalSettings ? (
              <ChevronDown className={`h-4 w-4 text-gray-400 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`} />
            ) : (
              <ChevronRight className={`h-4 w-4 text-gray-400 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`} />
            )}
          </button>
          {showGlobalSettings && (
        ...
        <div className={`border-b border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
          ...
          >
            <Search className={`h-4 w-4 text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`} />
            <span className={`flex-1 text-left font-medium text-gray-700 ${resolvedTheme === 'dark' ? "text-gray-200" : ""}`}>{t('pageSEO')}</span>
            {showPageSEO ? (
              <ChevronDown className={`h-4 w-4 text-gray-400 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`} />
            ) : (
              <ChevronRight className={`h-4 w-4 text-gray-400 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`} />
      ...
      {/* Add Section Button */}
      <div className={`p-3 border-t border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
        <Button
          onClick={onAddSection}
          variant="outline"
          size="sm"
          className={`w-full border-dashed border-gray-300 text-gray-500 hover:text-blue-600 hover:border-blue-400 bg-transparent hover:bg-blue-50 ${resolvedTheme === 'dark' ? "border-gray-600 text-gray-400 hover:text-white hover:border-gray-400 hover:bg-white/5" : ""}`}
        >
...
function SectionListItem({
  ...
  return (
    <div
      className={`border-b border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}
      draggable
    ...
    >
      ...
      >
        <GripVertical className={`h-3.5 w-3.5 text-gray-300 cursor-grab shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ${resolvedTheme === 'dark' ? "text-gray-600" : ""}`} />
        <IconComponent className={`h-4 w-4 text-blue-600 shrink-0 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`} />
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-medium truncate text-gray-800 ${resolvedTheme === 'dark' ? "text-white" : ""}`}>{label}</p>
          <p className={`text-xs text-gray-400 truncate ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`}>{variantLabel}</p>
        </div>
        {isActive ? (
          <ChevronDown className={`h-4 w-4 text-gray-400 shrink-0 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`} />
        ) : (
          <ChevronRight className={`h-4 w-4 text-gray-400 shrink-0 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`} />
        )}
      </div>

      {/* Section Content Editor (expanded) */}
      {isActive && (
        <div className={`px-3 pb-3 space-y-3 bg-gray-50 ${resolvedTheme === 'dark' ? "bg-white/5" : ""}`}>
          {/* Variant Selector */}
          {definition && definition.variants.length > 1 && (
            <div className="space-y-1.5">
              <Label className={`text-xs text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>{t('variant')}</Label>
              <Select
                value={section.section_variant}
                onValueChange={onUpdateVariant}
              >
                <SelectTrigger className={`h-8 text-xs bg-white border-gray-300 text-gray-800 ${resolvedTheme === 'dark' ? "bg-white/5 border-gray-600 text-white" : ""}`}>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          {definition?.contentFields.map((field) => (
            <div key={field.key} className="space-y-1.5">
              <Label className={`text-xs text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>{field.label}</Label>
              {field.type === 'text' && (
                <Input
                  ...
                  className={`h-8 text-xs bg-white border-gray-300 text-gray-800 placeholder:text-gray-400 ${resolvedTheme === 'dark' ? "bg-white/5 border-gray-600 text-white placeholder:text-gray-500" : ""}`}
                />
              )}
              {field.type === 'textarea' && (
                <Textarea
                  ...
                  placeholder={field.placeholder}
                  rows={2}
                  className={`text-xs bg-white border-gray-300 text-gray-800 placeholder:text-gray-400 resize-none ${resolvedTheme === 'dark' ? "bg-white/5 border-gray-600 text-white placeholder:text-gray-500" : ""}`}
                />
              )}
              {field.type === 'url' && (
                <Input
              ...
              {field.type === 'range' && (
                <div className="flex items-center gap-2">
                  <Slider
                    ...
                    className="flex-1"
                  />
                  <span className={`text-xs text-gray-500 min-w-[40px] text-right ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>
          ...
          {section.section_type === 'hero' && (
            <div className={`flex items-center justify-between pt-2 border-t border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
              <Label className={`text-xs text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>{t('bookingWidget')}</Label>
              <Switch
...
function ImageFieldPicker({ value, onChange }: { value: string; onChange: (url: string) => void }) {
  ...
  return (
    <>
      {value ? (
        <div className={`relative group rounded-md overflow-hidden border border-gray-300 ${resolvedTheme === 'dark' ? "border-gray-600" : ""}`}>
          <img
            src={value}
            alt={t('imageAlt')}
        ...
        <button
          onClick={() => setShowPicker(true)}
          className={`w-full h-16 flex flex-col items-center justify-center gap-1 rounded-md border border-dashed border-gray-300 text-gray-400 hover:border-blue-400 hover:text-blue-500 transition-colors bg-white ${resolvedTheme === 'dark' ? "border-gray-600 text-gray-500 hover:border-blue-500 hover:text-blue-400 bg-white/5" : ""}`}
        >
...
function GalleryItemsEditor({
  ...
  return (
    <div className={`space-y-2 pt-2 border-t border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
      <div className="flex items-center justify-between">
        <Label className={`text-xs text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>
          {t('imagesLabel', { count: items.length })}
        </Label>
        <button
          onClick={() => setShowPicker(true)}
          className={`flex items-center gap-1 text-[10px] text-blue-600 hover:underline ${resolvedTheme === 'dark' ? "text-blue-400" : ""}`}
        >
      ...
      {items.length > 0 ? (
        <div className="grid grid-cols-3 gap-1.5">
          {items.map((img) => (
            <div
              key={img.id}
              className={`relative group rounded overflow-hidden border border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700" : ""}`}
            >
              <img
                src={img.url}
                alt={img.alt}
        ...
        <p className={`text-[10px] text-gray-400 text-center py-2 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`}>
          {t('noImages')}
        </p>
      )}
        ...
        <div className="space-y-1.5">
          {items.map((t, idx) => (
            <div
              ...
              className={`rounded border border-gray-200 bg-white overflow-hidden ${resolvedTheme === 'dark' ? "border-gray-700 bg-white/5" : ""}`}
            >
              <div
                className={`flex items-center gap-2 px-2 py-1.5 cursor-pointer hover:bg-gray-50 ${resolvedTheme === 'dark' ? "hover:bg-white/5" : ""}`}
                onClick={() => setExpandedId(expandedId === t.id ? null : t.id)}
              >
                <MessageSquareQuote className="h-3 w-3 text-gray-400 shrink-0" />
                <span className={`text-[11px] flex-1 truncate text-gray-700 ${resolvedTheme === 'dark' ? "text-gray-300" : ""}`}>
                  {t.name || tr('noName')}
                </span>
                <button
              ...
              {expandedId === t.id && (
                <div className={`px-2 pb-2 space-y-1.5 border-t border-gray-100 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
                  <Input
                    value={t.name}
                    onChange={(e) => handleUpdate(t.id, { name: e.target.value })}
                    placeholder={tr('namePlaceholder')}
                    className={`h-7 text-[11px] mt-1.5 bg-white border-gray-200 text-gray-800 ${resolvedTheme === 'dark' ? "bg-white/5 border-gray-600 text-white" : ""}`}
                  />
                  <Input
                    value={t.company || ''}
                    onChange={(e) => handleUpdate(t.id, { company: e.target.value })}
                    placeholder={tr('companyPlaceholder')}
                    className={`h-7 text-[11px] bg-white border-gray-200 text-gray-800 ${resolvedTheme === 'dark' ? "bg-white/5 border-gray-600 text-white" : ""}`}
                  />
                  <Textarea
                    value={t.content}
                    ...
                    rows={2}
                    className={`text-[11px] bg-white border-gray-200 text-gray-800 resize-none ${resolvedTheme === 'dark' ? "bg-white/5 border-gray-600 text-white" : ""}`}
                  />
...
function CategorySelectorEditor({
  ...
  if (loading) {
    return (
      <div className={`pt-2 border-t border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
    ...
    <div className={`space-y-2 pt-2 border-t border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
          ...
          {selectedCats.map((cat, idx) => (
            ...
            >
              ...
              {cat.image_url ? (
                ...
                <div className={`w-6 h-6 rounded bg-gray-200 flex items-center justify-center shrink-0 ${resolvedTheme === 'dark' ? "bg-gray-700" : ""}`}>
                  <span className="text-[10px]">🏷️</span>
                </div>
              )}
              <span className={`text-xs text-gray-700 flex-1 truncate ${resolvedTheme === 'dark' ? "text-gray-300" : ""}`}>{cat.name}</span>
      ...
      {unselectedCats.length > 0 && (
        <div className="space-y-1 mt-2">
          <span className={`text-[10px] text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-500" : ""}`}>Disponibles</span>
          <div className={`max-h-40 overflow-y-auto space-y-1 rounded border border-gray-200 p-1 ${resolvedTheme === 'dark' ? "border-gray-700" : ""}`}>
            {unselectedCats.map((cat) => (
              <label
                key={cat.id}
                className={`flex items-center gap-2 p-1 rounded hover:bg-gray-100 cursor-pointer ${resolvedTheme === 'dark' ? "hover:bg-white/5" : ""}`}
              >
                <Checkbox
                  checked={false}
                ...
                {cat.image_url ? (
                  ...
                  <div className={`w-5 h-5 rounded bg-gray-200 flex items-center justify-center shrink-0 ${resolvedTheme === 'dark' ? "bg-gray-700" : ""}`}>
                    <span className="text-[9px]">🏷️</span>
                  </div>
                )}
                <span className={`text-xs text-gray-600 truncate ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>{cat.name}</span>
              </label>
            ))}
          </div>
        </div>
            ...
            >
              ...
              >
                ...
                {brand.logo_url ? (
                  ...
                  <div className={`h-6 w-10 bg-gray-200 rounded flex items-center justify-center shrink-0 ${resolvedTheme === 'dark' ? "bg-gray-700" : ""}`}>
  ...
  return (
    <div className={`pt-2 border-t border-gray-200 ${resolvedTheme === 'dark' ? "border-gray-700/50" : ""}`}>
      ...
      >
        ...
        <span className={`text-[11px] text-gray-500 flex-1 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>Espaciado</span>
        {isOpen ? (
          <ChevronUp className="h-3 w-3 text-gray-400" />
        ) : (
          <ChevronDown className="h-3 w-3 text-gray-400" />

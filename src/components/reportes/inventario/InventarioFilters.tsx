...
import type { InventarioFilters as InventarioFiltersType } from './inventarioReportService';

interface InventarioFiltersProps {
  filters: InventarioFiltersType;
  onFiltersChange: (filters: InventarioFiltersType) => void;
...
export function InventarioFilters({
  filters, onFiltersChange, branches, categories, isLoading,
}: InventarioFiltersProps) {
  const [expanded, setExpanded] = useState(false);

  const handleChange = (key: keyof InventarioFiltersType, value: string | number | null) => {
  ...
  const handleReset = () => {
    ...
    onFiltersChange({
      ...
      branchId: null, categoryId: null, stockStatus: null,
    });
  };

  const hasActive = filters.branchId || filters.categoryId || filters.stockStatus;
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div>
          <label className="text-[11px] text-gray-500 dark:text-gray-400 mb-1 block">Categoría</label>
          <Select value={filters.categoryId?.toString() || 'all'} onValueChange={(v) => handleChange('categoryId', v === 'all' ? null : Number(v))} disabled={isLoading}>
            <SelectTrigger className="h-9 text-sm bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600">
              <SelectValue placeholder="Todas" />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          <label className="text-[11px] text-gray-500 dark:text-gray-400 mb-1 block">Estado Stock</label>
          <Select value={filters.stockStatus || 'all'} onValueChange={(v) => handleChange('stockStatus', v === 'all' ? null : v)} disabled={isLoading}>
            <SelectTrigger className="h-9 text-sm bg-white dark:bg-gray-800 border-gray-300 dark:border-gray-600">
              <SelectValue placeholder="Todos" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Todos</SelectItem>
              <SelectItem value="critico">Crítico (sin stock)</SelectItem>
        ...
        <div>
          <label className="text-[11px] text-gray-500 dark:text-gray-400 mb-1 block">Desde (movimientos)</label>
      ...
      {expanded && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3 pt-3 border-t border-gray-100 dark:border-gray-700">
          <div>
            <label className="text-[11px] text-gray-500 dark:text-gray-400 mb-1 block">Hasta (movimientos)</label>

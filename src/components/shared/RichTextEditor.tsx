 ...
 * Sin dependencias externas. Genera HTML como salida.
 *
 * Uso:
 *   <RichTextEditor value={form.description} onChange={(html) => setForm(f => ({ ...f, description: html }))} placeholder="..." />
 *
 * Para visualizar el contenido generado, usar <HtmlContentRenderer html={value} />.
 */
...
export interface RichTextEditorProps {
  ...
  placeholder?: string;
  className?: string;
  /** Altura mínima del área editable (px). Por defecto 80. */
  minHeight?: number;
}
    ...
    <div className={cn('rounded-md border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 overflow-hidden', className)}>
      ...
      <div className="relative">
        ...
        <div
          ...
          onBlur={emitChange}
          style={{ minHeight }}
          className="prose-sm max-w-none px-3 py-2 text-sm text-gray-700 dark:text-gray-200 focus:outline-none [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_p]:my-1 [&_strong]:font-semibold [&_em]:italic [&_u]:underline"
        />
      </div>
    </div>
  );

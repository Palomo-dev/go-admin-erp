  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px]">
        ...
        {currentDriverId && (
          <div className={`flex items-center justify-between p-3 rounded-lg bg-blue-50 border border-blue-200 ${resolvedTheme === 'dark' ? "bg-blue-900/20 border-blue-800" : ""}`}>
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-medium">
                {getInitials(currentDriverName || '')}
              </div>
              <div>
                <p className={`text-sm font-medium text-blue-700 ${resolvedTheme === 'dark' ? "text-blue-300" : ""}`}>
            ...
            <Button
              ...
              disabled={assigning}
              className={`text-red-600 border-red-300 hover:bg-red-50 ${resolvedTheme === 'dark' ? "border-red-700 hover:bg-red-900/20" : ""}`}
            >
                ...
                return (
                  ...
                  >
                    {driver.avatar_url ? (
                      ...
                      <div className={`w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-white text-sm font-medium shrink-0 ${resolvedTheme === 'dark' ? "bg-gray-600" : ""}`}>
                        {getInitials(driver.name)}
                      </div>
                    )}
                    <div className="min-w-0 flex-1">
                      <p className={`text-sm font-medium text-gray-900 truncate ${resolvedTheme === 'dark' ? "text-white" : ""}`}>
                        {driver.name}
                        {isCurrent && (
                          <span className={`ml-2 text-xs text-blue-600 ${resolvedTheme === 'dark' ? "text-blue-400" : ""}`}>(actual)</span>
                        )}
                      </p>
                      <div className={`flex items-center gap-3 text-xs text-gray-500 ${resolvedTheme === 'dark' ? "text-gray-400" : ""}`}>
                        {driver.phone && (

...
interface ImportManifestsDialogProps {
  open: boolean;
...
export function ImportManifestsDialog({
  open,
  onOpenChange,
  onImport,
  isLoading = false,
  ...
  const downloadTemplate = () => {
    const template = 'manifest_date,manifest_type,vehicle_id,carrier_id,notes\n' +
      '"2025-01-30","delivery","uuid-vehiculo","uuid-carrier","Notas del manifiesto"\n';
    const blob = new Blob([template], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'plantilla_manifiestos.csv';
    a.click();
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Importe múltiples manifiestos desde un archivo CSV
          </DialogDescription>
        </DialogHeader>
...
export default ImportManifestsDialog;

      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
          {stats.map((stat) => (
            <div key={stat.label} className="text-center">
              ...
              <p className={`text-xs text-gray-500 ${themeClass("", "text-gray-400")}`}>{stat.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className={`text-gray-500 ${themeClass("", "text-gray-400")}`}>Progreso de entregas</span>
            <span className={`font-medium text-gray-900 ${themeClass("", "text-white")}`}>{progress.toFixed(0)}%</span>
          </div>
          ...
          <div className="flex items-center justify-between text-xs">
            <span className={`text-green-600 ${themeClass("", "text-green-400")}`}>
              {delivered} entregados ({successRate.toFixed(0)}% éxito)
            </span>
            <span className={`text-red-600 ${themeClass("", "text-red-400")}`}>
              {failed} fallidos
            </span>
          </div>
        </div>
        ...
        {/* Totales adicionales */}
        <div className={`grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-gray-100 ${themeClass("", "border-gray-800")}`}>
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className={`text-xs text-gray-500 ${themeClass("", "text-gray-400")}`}>Peso total</p>
            </div>
          </div>
          {manifest.total_cod_amount && Number(manifest.total_cod_amount) > 0 && (
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className={`text-xs text-gray-500 ${themeClass("", "text-gray-400")}`}>COD a recaudar</p>
              </div>
            </div>
          )}
        </div>

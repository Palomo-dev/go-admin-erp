...
import { Loader2, Truck, DollarSign, MapPin, Calendar, Tag, Globe } from 'lucide-react';
import type {
...
export function ShippingRateDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!payload.rate_code) delete payload.rate_code;
      ...
      <DialogContent className="max-w-3xl max-h-[92vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <div className={`flex items-center gap-2 text-sm font-medium text-gray-700 ${themeClass("", "text-gray-300")}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => handleChange('carrier_id', v === 'none' ? undefined : v)}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Sin transportador específico" />
                  </SelectTrigger>
                  <SelectContent>
          ...
          {/* Sección: Precios */}
          <div className={`space-y-4 pt-4 border-t ${themeClass("", "border-gray-700")}`}>
            <div className={`flex items-center gap-2 text-sm font-medium text-gray-700 ${themeClass("", "text-gray-300")}`}>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="rate_per_kg" type="number" min="0" step="0.01" value={formData.rate_per_kg || 0} onChange={(e) => handleChange('rate_per_kg', parseFloat(e.target.value) || 0)} />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="min_charge" type="number" min="0" step="0.01" value={formData.min_charge || 0} onChange={(e) => handleChange('min_charge', parseFloat(e.target.value) || 0)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="min_weight_kg">Peso mín. (kg)</Label>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="fuel_surcharge_percent" type="number" min="0" max="100" step="0.1" value={formData.fuel_surcharge_percent || 0} onChange={(e) => handleChange('fuel_surcharge_percent', parseFloat(e.target.value) || 0)} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="insurance_percent">Seguro (%)</Label>
                <Input id="insurance_percent" type="number" min="0" max="100" step="0.1" value={formData.insurance_percent || 0} onChange={(e) => handleChange('insurance_percent', parseFloat(e.target.value) || 0)} />
              </div>
            </div>
          </div>
          ...
          <div className={`space-y-4 pt-4 border-t ${themeClass("", "border-gray-700")}`}>
            ...
            {(formData as any).show_on_website && (
              <div className={`p-3 border rounded-lg bg-blue-50 space-y-3 ${themeClass("", "bg-blue-900/10")}`}>
                <div className="flex items-center justify-between">
                  <div>
          ...
          {/* Botones */}
          <div className={`flex flex-col-reverse sm:flex-row justify-end gap-3 pt-4 border-t sticky bottom-0 bg-white ${themeClass("", "border-gray-700 bg-gray-900")}`}>
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="w-full sm:w-auto">
              Cancelar
            </Button>
            <Button type="submit" disabled={isLoading || !formData.rate_name} className="w-full sm:w-auto">
              {isLoading ? (
                <>

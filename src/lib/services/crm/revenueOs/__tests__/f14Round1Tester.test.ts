...
/**
 * F14 — ronda 1, pruebas del tester con FILAS REALES de la base (2026-09-15,
 * solo lectura por MCP; solo ids de organización). Cada fixture es la salida
 * literal de la RPC y, al lado, la cifra recalculada a mano con SQL directo.
 *
 * Lo que se comprueba: que los módulos puros conviertan esas filas en lo que
 * la UI pinta, y que las limitaciones de las RPC (no del código de F14) queden
 * documentadas y no disfrazadas de cero.
 *
 * Ronda 2 (2026-09-15): `fn_revenue_metrics` fue reescrita (espina de meses,
 * cobrado sin exigir oportunidad, `revenue_collected_linked`, `invoices_paid`).
 * Las fixtures llevan las columnas nuevas con los valores reales y los casos
 * marcados «LIMITACIÓN RPC» / «HALLAZGO» pasan a exigir la cifra correcta:
 * org 135 cobrado 16 970 600 (no 0), org 2 ARPA 90 737,50 (no 104 892,83),
 * conversión global 25 % (no 0 %). Se conservan las 21 pruebas del tester.
 */
 ...
 * completa está en `f14Round2.test.ts`). Los meses sin oportunidades ahora
...
describe('F14 r1 tester — rango por defecto', () => {
...
describe('F14 r1 tester — org 2 (1 ganada, 2 perdidas): RPC vs SQL a mano vs UI', () => {
...
describe('F14 r1 tester — embudo org 2, pipeline por defecto', () => {
...
describe('F14 r1 tester — forecast org 2, pipeline por defecto (probabilidades reales 0/0/50/0/0/90)', () => {
  const stages = [
    { id: 's0', probability: 0 },
    { id: 's1', probability: 0 },
...
describe('F14 r1 tester — cohortes org 135 (45 + 5 clientes) el 2026-09-15', () => {
...
describe('F14 r1 tester — matemática comercial con las filas reales de org 135 y org 2', () => {
...
describe('F14 r1 tester — supervivientes de la campaña de mutaciones (T07, T11, T16)', () => {

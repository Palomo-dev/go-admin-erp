...
export default class FacebookChannelService {
  ...
  async getChannel(channelId: string): Promise<FacebookChannel | null> {
    ...
    if (error) {
      console.error('Error obteniendo canal Facebook:', error);
      return null;
    }
  ...
  async updateChannel(channelId: string, updates: Partial<FacebookChannel>): Promise<boolean> {
    await this.setOrgContext();
    ...
    if (error) {
      console.error('Error actualizando canal Facebook:', error);
      return false;
    }
  ...
  async saveCredentials(channelId: string, credentials: FacebookCredentials['credentials']): Promise<boolean> {
    await this.setOrgContext();
  ...
  async validateWebhook(channelId: string): Promise<{ valid: boolean; message: string }> {
    ...
    if (!creds?.credentials?.page_id || !creds?.credentials?.page_access_token) {
  ...
  async getStats(channelId: string): Promise<FacebookStats> {
    await this.setOrgContext();

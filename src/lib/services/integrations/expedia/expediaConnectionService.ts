 ...
 */
export function createExpediaConnectionService(clients: ChannelManagerClients) {
  const supabase = clients.db;
  const expediaAuthService = createExpediaAuthService(clients);
...
export type ExpediaConnectionService = ReturnType<typeof createExpediaConnectionService>;

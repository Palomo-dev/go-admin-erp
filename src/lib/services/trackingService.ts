...
class TrackingService {
  ...
  private async enrichEventsWithReferences(events: TrackingEvent[], organizationId: number): Promise<TrackingEvent[]> {
    const tripIds = [...new Set(events.filter(e => e.reference_type === 'trip').map(e => e.reference_id))];
    const shipmentIds = [...new Set(events.filter(e => e.reference_type === 'shipment').map(e => e.reference_id))];

 ...
 */
async function fixPaymentNotificationMethod(
  ...
  try {
    ...
    console.warn('[webOrderServerConfirmation] No se pudo corregir la notificación de pago:', err);
  }
}
  ...
  async confirmOrder(
    ...
    if (invoiceId) {
      try {
        ...
        // usa 'completed'. Aceptar ambos para evitar duplicar el pago (y la
        // notificación que dispara el trigger trg_notify_payment_registered).
        ...
        if (existingPayment) {
          ...
          await fixPaymentNotificationMethod(
            supabase,

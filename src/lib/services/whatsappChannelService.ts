...
export default class WhatsAppChannelService {
  ...
  async getChannel(channelId: string): Promise<WhatsAppChannel | null> {
  ...
  async updateChannel(channelId: string, updates: Partial<WhatsAppChannel>): Promise<boolean> {
    await this.setOrgContext();
    ...
    if (error) {
      console.error('Error actualizando canal WhatsApp:', error);
      return false;
    }
  ...
  async saveCredentials(channelId: string, credentials: WhatsAppCredentials['credentials']): Promise<boolean> {
    await this.setOrgContext();
  ...
  async getStats(channelId: string): Promise<WhatsAppStats> {
    await this.setOrgContext();
    ...
    return {
      ...
      deliveredRate: 95, // Placeholder - calcular desde message_events
      failedCount: 0 // Placeholder - calcular desde message_events
    };
  }

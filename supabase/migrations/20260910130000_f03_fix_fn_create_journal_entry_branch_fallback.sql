-- F-03: Arreglar fallback de sucursal en fn_create_journal_entry
--
-- Antes: cuando p_branch_id era NULL, 0 o invalido, caia a
--   SELECT MIN(id) FROM branches WHERE organization_id = ...
-- que aterriza todos los asientos en la sucursal de menor id.
--
-- Ahora: respeta p_branch_id cuando es valido. Solo cae al default
-- cuando viene realmente NULL/0/invalido, y el default es la sucursal
-- marcada como is_main=true (sucursal principal), no la de menor id.
-- Si no hay sucursal principal, cae a MIN(id) como ultimo recurso.
--
-- Ademas: anade SET search_path = public, pg_temp para cerrar el vector
-- de escalada en SECURITY DEFINER (F-11).
--
-- Reversible: ver supabase/rollbacks/20260910130000_f03_fix_fn_create_journal_entry_branch_fallback_rollback.sql
...
BEGIN
    ...
    -- F-03: Validar branch_id: NULL, 0, o que no pertenezca a la organizacion -> fallback
    -- El fallback ahora usa la sucursal principal (is_main=true) en vez de MIN(id).
    IF p_branch_id IS NULL OR p_branch_id = 0 OR NOT EXISTS (
        ...
        -- Buscar sucursal principal de la organizacion
        SELECT id INTO v_branch_id
        FROM branches
        WHERE organization_id = p_organization_id AND is_main = true AND is_active = true
        LIMIT 1;

        -- Si no hay sucursal principal, caer a MIN(id) como ultimo recurso

-- pos_checkout_v1: tipo de propina alineado con el camino de respaldo.
--
-- Antes la RPC fijaba tips.tip_type = 'card' SOLO si el primer elemento del
-- sobre de pagos era 'card'; una propina cobrada por QR (Bre-B/Nequi/Bold),
-- datáfono o transferencia quedaba como 'cash' (y 'transfer' no cabe en el
-- CHECK de tips). Ahora: primer pago con importe > 0; 'cash' → 'cash', todo lo
-- demás → 'card'. Misma regla que POSService.checkout (src/lib/services/
-- posService.ts). Sin cambios de firma ni de contrato del sobre; no toca datos.
-- Reversión: supabase/rollbacks/20260922120000_pos_checkout_v1_tip_type_no_efectivo_rollback.sql
-- (vuelve a la definición de 20260921100000).

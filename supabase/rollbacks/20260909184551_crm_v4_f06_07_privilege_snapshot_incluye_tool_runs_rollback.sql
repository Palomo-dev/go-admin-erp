-- ============================================================
-- ROLLBACK de 20260909184551_crm_v4_f06_07_privilege_snapshot_incluye_tool_runs
-- ============================================================
...
-- Restaura fn_f6_privilege_snapshot a la versión de f06_05 (sin voice_agent_tool_runs).
-- ============================================================
  ...
  SELECT jsonb_build_object(
    ...
    'tables', COALESCE((
      SELECT jsonb_object_agg(c.relname, jsonb_build_object('rls', c.relrowsecurity, 'acl', COALESCE(c.relacl::text, 'DEFAULT')))
       ...
       WHERE n.nspname = 'public'
         AND c.relname IN ('voice_agent_call_attempts', 'voice_agents', 'voice_agent_calls', 'voice_agent_campaigns', 'stage_agents', 'voices')
    ), '{}'::jsonb),
    'policies', COALESCE((
      SELECT jsonb_agg(jsonb_build_object('table', pol.tablename, 'cmd', pol.cmd, 'name', pol.policyname)
       ...
       WHERE pol.schemaname = 'public' AND pol.tablename IN ('voice_agent_call_attempts', 'voices', 'stage_agents')
    ), '[]'::jsonb)
  );
$$;

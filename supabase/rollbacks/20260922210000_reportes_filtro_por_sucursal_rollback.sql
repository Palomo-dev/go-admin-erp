-- ============================================================================
-- ROLLBACK de 20260922210000_reportes_filtro_por_sucursal.sql
-- ============================================================================
-- Restaura las 9 funciones de reportes a su firma de 3 argumentos
-- (o 2 en los aging), tal como estaban antes de la migración, y elimina la
-- variante de 4 argumentos. Los cuerpos son copia literal de
-- pg_get_functiondef() tomada antes de aplicar la migración.
--
-- ATENCIÓN: al revertir, el frontend vuelve a romperse con 404 de PostgREST
-- si sigue enviando `p_branch_id`. Revertir esta migración exige revertir
-- también src/lib/services/reportes/modulos/{ventas,finanzas,inventario}Reports.ts.
-- ============================================================================

-- ---------------------------------------------------------------------------
-- 1. fn_reporte_cierre_caja
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_cierre_caja(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_cierre_caja(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
ALTER FUNCTION public.fn_reporte_cierre_caja(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_cierre_caja(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_cierre_caja(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 2. fn_reporte_ventas_resumen
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_ventas_resumen(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_ventas_resumen(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_ventas_resumen(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_ventas_resumen(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_ventas_resumen(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 3. fn_reporte_ventas_por_hora
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_ventas_por_hora(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_ventas_por_hora(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_ventas_por_hora(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_ventas_por_hora(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_ventas_por_hora(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 4. fn_reporte_cxc_aging
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_cxc_aging(bigint, date, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_cxc_aging(p_organization_id bigint, p_as_of date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_cxc_aging(bigint, date) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_cxc_aging(bigint, date) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_cxc_aging(bigint, date) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 5. fn_reporte_cxp_aging
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_cxp_aging(bigint, date, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_cxp_aging(p_organization_id bigint, p_as_of date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_cxp_aging(bigint, date) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_cxp_aging(bigint, date) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_cxp_aging(bigint, date) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 6. fn_reporte_flujo_efectivo
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_flujo_efectivo(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_flujo_efectivo(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_flujo_efectivo(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_flujo_efectivo(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_flujo_efectivo(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 7. fn_reporte_impuestos
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_impuestos(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_impuestos(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_impuestos(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_impuestos(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_impuestos(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 8. fn_reporte_movimientos_inventario
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_movimientos_inventario(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_movimientos_inventario(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_movimientos_inventario(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_movimientos_inventario(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_movimientos_inventario(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- 9. fn_reporte_rotacion_inventario
-- ---------------------------------------------------------------------------
DROP FUNCTION IF EXISTS public.fn_reporte_rotacion_inventario(bigint, timestamptz, timestamptz, bigint);

CREATE OR REPLACE FUNCTION public.fn_reporte_rotacion_inventario(p_organization_id bigint, p_from timestamp with time zone, p_to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
ALTER FUNCTION public.fn_reporte_rotacion_inventario(bigint, timestamptz, timestamptz) OWNER TO postgres;
GRANT EXECUTE ON FUNCTION public.fn_reporte_rotacion_inventario(bigint, timestamptz, timestamptz) TO PUBLIC;
GRANT EXECUTE ON FUNCTION public.fn_reporte_rotacion_inventario(bigint, timestamptz, timestamptz) TO anon, authenticated, service_role;
